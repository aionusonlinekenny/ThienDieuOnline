...
PHANTRAM_NGUHANH = 40;			  				   --|
PHANTRAM_UPLEVEL = 40;			  			 	   --|
PHANTRAM_CHANGEOPTION = 50;		  				   --|
-- level_option_vip = random(9,10);   				   --|
level_option_vip = random(1,6);   				   --|
...
function Covert_NguHanh(nItem,nItemGenre,nDetail,nParticur,nLevel,nSeries,nLucky,nSeriesNew)
	if tylephantram(PHANTRAM_NGUHANH) == 1 then
	return end
	RemoveItem(nItem,1);
	local FronOP1 ={};
	local FronOP2 ={};
	...
	nIndex = AddItem(0,0,nDetail,nParticur,nLevel,nSeriesNew,0,6);	
	local value1_3_n,
		         value2_3_n,
		         value3_3_n,
		         value4_3_n,
...
function UpLevel_Item(nItem,nItemGenre,nDetail,nParticur,nLevel,nSeries,nLucky)
	if tylephantram(PHANTRAM_UPLEVEL) == 1 then
	return end
	RemoveItem(nItem,1);
	local MA = {};
	local VA = {};
...
function changeOption_Item(nItem,nItemGenre,nDetail,nParticur,nLevel,nSeries,nLucky)
	...
	return end
	RemoveItem(nItem,1);
	nIndex = AddItem(0,0,nDetail,nParticur,nLevel,nSeries,nSeries,level_option_vip);
	AddItemID(nIndex);
...
local nRan = random(1,100);
	if (nRan > nPhantram) then
		Msg2Player("<color=blue> §¹i hiÖp ®· thÊt b¹i, xin thö l¹i, tæn thÊt 1 thñy tinh. Tû lÖ thµnh c«ng lµ "..100-nPhantram.."%");
		return 1
	else
		return 0
	end
