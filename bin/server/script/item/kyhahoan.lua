...
function main(sel)
	...
	if(nTaskValue >= 3) then
		Msg2Player("Mçi ngµy mçi ng­êi chØ ®­îc sö dung 3 Kú Hµ Hoµn");
	return end;
	SetTask(TASK_RESET3, SetNumber(1,nTotalTask, 7, nTaskValue+1))
	nTotalTask = GetTask(TASK_RESET);
	nTaskValue = GetNumber(1,nTotalTask,11);
	SetTask(TASK_RESET, SetNumber(1,nTotalTask, 11, nTaskValue+1))
	Talk(1,"","Thu ®­îc thªm c¬ héi 1 lÇn tham gia ho¹t ®éng <Phong L¨ng §é>!");
	RemoveItem(sel,1);
end