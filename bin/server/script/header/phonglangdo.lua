...
function release_npcphonglang()
	...
	nNpcIdx = AddNpcNew(724,100,337,NPC_PHONGLANG[i][1],NPC_PHONGLANG[i][2],DIE_PHONGLANG,nil,nil,
		1,RANDOMC(0,1,2,3,4),NPCEXP,50000,nil,300,50,50,55,nil,500,25,nil,DROP_PHONGLANG);
		if(nNpcIdx > 0) then
		AddMSNpc(1,nNpcIdx);
		end
	end
	Msg2MSAll(1,"Thuy“n Æ∑ rÍi b’n, qu˝ hi÷p kh∏ch xin Ên Æﬁnh chÁ ngÂi!");
end;
...
function release_phonglangboss()
	...
	nil,"ThÒy t∆c Æ«u l‹nh",1,RANDOMC(0,1,2,3,4),NPCEXP*5,1000000,nil,500,80,50,55,nil,12*1080,50,2,DROP_PHONGLANG);
	if(nNpcIdx > 0) then
		AddMSNpc(1,nNpcIdx);
	end
	end
	Msg2MSAll(1,"<color=yellow> ThÒy t∆c Æ«u l‹nh xu t hi÷n<color>");
end;
