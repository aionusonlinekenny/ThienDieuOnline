...
function OnDeath(nDesNpcIndex)
	if(SubWorld == SubWorldID2Idx(380)) then
		...
		if(nPlayerDataIdx > 0 and nDesPlayerData > 0 and nOnline == 1 and nDesOnline == 1) then	--co ton tai. va ca 2 dang online trong chien truong
			...
			local nCamKill=GetCurCamp()
			if (nCamKill==nCamDie) then
			return end
			...
			if (nRank < 5) then
				for i=1,getn(EXPAND_RANK) do
					...
					SetRankEx(EXPAND_RANK[i][nGroup+3],"255,0,240")
					end
				end
			end
			local szBeKill = "";
			...
			if (GetCurCamp()  == 1) then
				szBeKill  = "Phe Tèng  "..tbRANKNAME[nRank].."<color=yellow> "..szDesName.." <color>h¹ träng th­¬ng ng­êi phe Kim  "..tbRANKNAME[nDieRank].." <color=yellow>"..szName.."<color>, tæng PK lµ "..GetPMParam(1,nDesPlayerData,5);
			else
				szBeKill  = "Phe Kim  "..tbRANKNAME[nRank].."<color=yellow> "..szDesName.." <color> h¹ träng th­¬ng ng­êi phe Tèng  "..tbRANKNAME[nDieRank].."<color=yellow> "..szName.."<color>, tæng PK lµ "..GetPMParam(1,nDesPlayerData,5);
			end
			Msg2MSAll(1,szBeKill, -2);--thong bao toan nguoi choi
			Msg2Player("<color=Pink>Chóc mõng! B¹n h¹ gôc "..tbRANKNAME[nDieRank].." <color=yellow>"..szName.."<color>, tæng PK lµ "..GetPMParam(1,nDesPlayerData,5).."<color>");
		end
	end
end;