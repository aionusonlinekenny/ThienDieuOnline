--------========================================================================================----------
--------========================================================================================----------
--------=======================================ANHVIETDONGNAI=====================================-----------
--------========================================================================================----------
--------========================================================================================----------

Include("\\script\\checkIPtongkim.lua");
Include("\\script\\serverscript.lua");
Include("\\script\\lib\\worldlibrary.lua");
Include("\\script\\header\\taskid.lua");
Include("\\script\\header\\npcfile.lua");
Include("\\script\\header\\bosstieuhk.lua");
Include("\\script\\header\\vuotai.lua");
Include("\\script\\header\\tongkim.lua");
Include("\\script\\header\\loginao.lua");
Include("\\script\\event\\demhuyhoang\\event.lua");
Include("\\script\\event\\hoadang\\lib.lua");
Include("\\script\\player\\playerlogin.lua");
Include("\\script\\header\\time.lua");
Include("\\script\\feature\\congthanh\\npccongthanh.lua")
Include("\\datascript\\congthanh\\danhsachcongthanh.lua")
Include("\\script\\feature\\songbac\\chusongbac.lua")


MSG_LIENDAU = "Vâ l©m kiÖt xuÊt %d ®· ®Õn giai ®o¹n b¸o danh. C¸c cao thñ mau nhanh ch©n ®¨ng ký thi ®Êu!"
Yr,Mth,Dy,Hr,Mn,Se = 0,0,0,0,0,0;

function main()
	Yr,Mth,Dy,Hr,Mn,Se = GetTime();
	if ((Hr == 0) and (Mn == 0)) then
		RLSvTask()
	         end
		addbosstieuhk()
		--huy hoang
		addhathuyhoang()
		--thach thuc thoi gian
		active_vuotai()
		--lien dau
		-- active_liendau()
		--phonglangdo
		active_phonglangdo()
		--hoa dang
		-- active_hoadang()
		--xo so'
		-- active_somayman()
		-- tongkim nªn ®Æt ë dßng cuèi cïng
                               batquaiTK()
                               kichraCTC()
                                RunTimer123()
                                -- baotrisv() 
		    active_tongkim()
                                -- ketquabaucuatom()
                            
                                            

	
	PosNews();
	-----print(">>>>>>>>>>>>>>>> Online : [ "..GetPlayerCount().." ] ac <<<<<<<<<<<<<<<<<<<")
end;


----------------------==============================================================--------------------------------------------------------------
function ketquabaucuatom()
 if (Mn == 00) or (Mn == 5) or (Mn == 10) or (Mn == 15) or (Mn == 20) or (Mn == 25) or (Mn == 30) or (Mn == 35) or (Mn == 40) or (Mn == 45) or (Mn == 50) or (Mn == 55) then
RunTimer123456()
        return end
end


function baotrisv() 
if (Hr == 11 and Mn == 55) then
Msg2SubWorld("<color=yellow>SÏ b¶o tr× sv trong 1 phót n÷a");
return end
if (Hr == 11 and Mn == 56) then
testConnect()
 
       return end
---------------------------------------------------------
 if (Hr == 17 and Mn == 55) then
 Msg2SubWorld("<color=yellow>SÏ b¶o tr× sv trong 1 phót n÷a");
CHECK_TK = {{"",""}}
danhsach = TaoBang(CHECK_TK,"CHECK_TK","")
 SaveData("script/checkIPtongkim.lua",danhsach) 
        return end
 if (Hr == 17 and Mn == 56) then
testConnect()
 
        return end
--------------------------------------------------------
if (Hr == 23 and Mn == 55) then
Msg2SubWorld("<color=yellow>SÏ b¶o tr× sv trong 1 phót n÷a");
return end
if (Hr == 23 and Mn == 56) then
testConnect()
 
       return end
end
---========================================================---
function testConnect()
 
local reload = openfile("ham.txt", "w");
 
write(reload, "reload");
 
closefile(reload);

end
---=====================================================-----

function RunTimer123()                 --------------------------bËt chÕ ®é liªn quan tíi c«ng thµnh
  if (Hr == 18 and Mn == 58) then
ReloadAllScript()
              return end
    if (Hr == 19 and Mn == 00) then
thongbaodaugia()
	return end
 if (Hr == 20 and Mn == 00) then
Msg2SubWorld("<color=yellow>C«ng thµnh chiÕn gi÷a bang c«ng thµnh<color> <color=green> ["..DSCTC[vitrictc()][1][1].."] <color>vµ bang thñ thµnh <color=green> ["..DSCTC[vitrictc()][2][1].."] <color>b¾t ®Çu !")
AddCountNews("C«ng thµnh chiÕn gi÷a bang c«ng thµnh <color=green>["..DSCTC[vitrictc()][1][1].."]<color> vµ bang thñ thµnh <color=yellow>["..DSCTC[vitrictc()][2][1].."]<color> ®· b¾t ®Çu ! ",1)
AddNpcNew(530,100,221,1723*32,3280*32,"\\script\\global\\lastdamage\\ctc_longtru.lua",
   5,"Thñ Long Nguyªn Trô",1,RANDOMC(0,1,2,3,4),0,50000,25,500,10,1,10,nil,600,20,nil,nil);
AddNpcNew(530,100,221,1655*32,3347*32,"\\script\\global\\lastdamage\\ctc_longtru.lua",
   5,"Thñ Long Nguyªn Trô",1,RANDOMC(0,1,2,3,4),0,50000,25,500,10,1,10,nil,600,20,nil,nil);
AddNpcNew(530,100,221,1590*32,3411*32,"\\script\\global\\lastdamage\\ctc_longtru.lua",
   5,"Thñ Long Nguyªn Trô",1,RANDOMC(0,1,2,3,4),0,50000,25,500,10,1,10,nil,600,20,nil,nil);

AddNpcNew(532,100,221,1662*32,3473*32,"\\script\\global\\lastdamage\\ctc_cuaa.lua",
   5,"B×nh Giang M«n",1,RANDOMC(0,1,2,3,4),0,50000,25,500,10,1,10,nil,600,20,nil,nil);
AddNpcNew(532,100,221,1728*32,3408*32,"\\script\\global\\lastdamage\\ctc_cuab.lua",
   5,"TÞnh D­¬ng M«n",1,RANDOMC(0,1,2,3,4),0,50000,25,500,10,1,10,nil,600,20,nil,nil);
AddNpcNew(532,100,221,1795*32,3342*32,"\\script\\global\\lastdamage\\ctc_cuac.lua",
   5,"§Þnh Xuyªn M«n",1,RANDOMC(0,1,2,3,4),0,50000,25,500,10,1,10,nil,600,20,nil,nil);

--AddTrapEx2(221,1662,3474,8,"\\script\\global\\lastdamage\\ctc_cua1.lua")
--AddTrapEx2(221,1728,3409,8,"\\script\\global\\lastdamage\\ctc_cua2.lua")
--AddTrapEx2(221,1795,3342,8,"\\script\\global\\lastdamage\\ctc_cua3.lua")


	return end
end

function kichraCTC()
 if (Hr == 20 or Hr == 22 and Mn == 00) then
for i=1,2000 do
	PlayerIndex = i
	w,x,y = GetWorldPos()
		if w == 221 or w == 221 or w == 221 then
                                            StopTimer();
                                           NewWorld(53,200*8,200*16)
			 SetFightState(0)
                                         SetTask(TASK_LONGTRU,0)
                                        SetTask(TASK_CUACTC1,0)
                                        SetTask(TASK_CUACTC2,0)
                                        SetTask(TASK_CUACTC3,0)
			Msg2Player("B¹n ®· trë vÒ Ba L¨ng HuyÖn ")
                                     KickOutSelf()
		break

		end
	end

end
end



-----------------------------------------------------------------------------------------------------
function RLSvTask()
	if(Hr ~= 0 or Mn ~= 0) then
	return end
	SetData(DATA_DCHAVERES,0);
	for i=1,1199 do
		PlayerIndex = i;
-------------------------------------------Resset c¸c task trong ngµy---------------------------
                            SetTask(TASK_THUTHUE ,0)  
                            SetTask(TASK_BAUCUATOM,0)
                            SetGlbMission(11,0)
-----------------------------------------------------------------------------------------------------
		if(GetPlayerNpcIdx() > 0) then
		BatDauVaoGame()
		end
	end
end

function addbosstieuhk() 
    if (Hr == 9 and Mn == 10 )then
	releasebosstieu()
	return end
    if (Hr == 17 and Mn == 10) then
	releasebosstieu()
	return end
	if (Hr == 21 and Mn == 10) then
	releasebosstieu()
	return end
	if (Hr == 23 and Mn == 10) then
	releasebosstieu()
	return end
end;

function addhathuyhoang()
--hat huy hoang
	if(Hr == 12 and Mn == 0) then
	addhhh()
	return end
	if(Hr == 12 and Mn == 10) then
	addhhh()
	return end
	if(Hr == 12 and Mn == 20) then
	addhhh()
	return end
end;
function addquahuyhoang()
--qua huy hoang
	if(Hr == 12 and Mn == 5) then
	addqhh()
	return end
	if(Hr == 12 and Mn == 15) then
	addqhh()
	return end
	if(Hr == 12 and Mn == 25) then
	addqhh()
	return end
end;

function active_vuotai()
	for i=1,getn(VUOTAI_TIMER) do
		if(Hr == VUOTAI_TIMER[i][1] and Mn == VUOTAI_TIMER[i][2]) then
		AddCountNews(10221,3)
		local nSubWorldId;
		for i=1,getn(MAP_VUOTAI) do
			nSubWorldId = SubWorldID2Idx(MAP_VUOTAI[i])
			if (nSubWorldId >= 0) then
			SubWorld = nSubWorldId;
			OpenMission(1)
			StartMissionTimer(1,1, 5400)--so phut de chinh thuc bat dau -- 5 phuts
			end
		end
	end
end
	--thong bao bat dau
	if(Mn == 35) then
		--AddCountNews(10247,3)
	end
end;

function active_liendau()
	if (Hr == 18)	then
		if(Mn == 0 or Mn == 15 or Mn == 30 or Mn == 45) then
			Msg2SubWorld(format(MSG_LIENDAU,floor(Mn/15)+1))
			local nSubWorldId = SubWorldID2Idx(396)
			if (nSubWorldId >= 0) then
			SubWorld = nSubWorldId;
			OpenMission(1)
			StartMissionTimer(1,1, 30*18)--so phut de chinh thuc bat dau
			end
		end
	-- elseif(Hr == 19 and Mn == 0) then
		-- SaveDataFile();
	end
end;

function active_phonglangdo()
for i=1,getn(PHONGLANGDO_TIMER) do
if(Hr == PHONGLANGDO_TIMER[i][1] and Mn == PHONGLANGDO_TIMER[i][2]) then
		AddCountNews(10137,3)
		local nSubWorldId;
			nSubWorldId = SubWorldID2Idx(337)
			if (nSubWorldId >= 0) then
			SubWorld = nSubWorldId;
			OpenMission(1)
			StartMissionTimer(1,1, 9720)--so phut de chinh thuc bat dau - 9 phut
			end  
	elseif (Mn >1 and Mn < 10) then
			nSubWorldId = SubWorldID2Idx(337)
			if (nSubWorldId >= 0) then
			SubWorld = nSubWorldId;
				--Msg2MSAll(1,"ThuyÒn sÏ rêi bÕn trong "..(10-Mn).." phót n÷a!");
				if (Mn == 2 or Mn == 3 or Mn == 4 or Mn == 5 or Mn == 6 or Mn == 7 or Mn == 8 or Mn == 9) then
				--AddCountNews("ThuyÒn sÏ rêi bÕn trong "..(10-Mn).." phót n÷a!",3);
				end
			end
	end	
end
end



function active_somayman()
	if(Hr == 16 and Mn == 45) then
		SetData(DATA_DCHAVERES,1);
		SetData(DATA_DATCUOC,GetDataInt(DATA_DATCUOC)+1);
		
		local nDataID=DATA_DCBEGIN;
		if(nDataID > 1) then
		repeat
			local nRandNum = random(99);
			local nReplace=1;
			for i =DATA_DCBEGIN,(DATA_DCBEGIN+nDataID) do
				if(GetDataInt(i) == nRandNum) then
					nReplace=0;
					i=DATA_DCBEGIN+nDataID;
				end;
			end;
			if(nReplace==1) then
			SetData(nDataID,nRandNum);
			nDataID=nDataID+1;
			end;
		until (nDataID >=DATA_DCEND)
		end;
		for i=DATA_DCBEGIN,DATA_DCEND do
		SetData(i,random(99));
		end
		Msg2SubWorld("HiÖn t¹i ®· cã kÕt qu¶ 'con sè may m¾n'. Quý ®ång ®¹o xem t¹i <color=yellow>ThÇn Tµi<color> (BiÖn Kinh 220/190).");
	end;
end;

function batquaiTK()
	if(Hr == 11 or Hr == 13 or Hr == 21 or Hr == 23) and (Mn == 01) then     
              release_npctk();
	Msg2SubWorld("Qu¸i Tèng Kim ®· xu©t hiÖn");
	end	
end


function active_tongkim()
	--Check thoi gian
	local nSubWorldId = SubWorldID2Idx(380)
	if (nSubWorldId >= 0) then
	for i=1,getn(TONGKIM_TIMER) do
		if(Hr == TONGKIM_TIMER[i][1] and Mn == 0) then
			SubWorld = nSubWorldId;
			OpenMission(1,TONGKIM_KIND[TONGKIM_TIMER[i][2]],2,0,1,3)
			SetMission(0,TONGKIM_TIMER[i][2])
			AddCountNews("TrËn ®¸nh Tèng Kim h×nh thøc "..GetMissionName(1).." ®ang hÕt søc c¨ng th¼ng, tr­íc m¾t ®ang ë giai ®o¹n b¸o danh, nh÷ng ai muèn tham chiÕn xin h·y nhanh chãng ®Õn T­¬ng D­¬ng hoÆc Chu Tiªn trÊn ®¨ng ký b¸o danh; HoÆc sö dông Tèng Kim chiªu th­ ®Ó nhanh chãng ®Õn ®iÓm b¸o danh. §iÒu kiÖn tham gia: ®¼ng cÊp tõ 40. phÝ b¸o danh 3000 l­îng",3);
			AddCountNews2(10222,3)
			StartMissionTimer(1,1, 2*1080)--so phut de chinh thuc bat dau
			if(GetMissionV(0) == 1) then
				StartMissionTimer(1,2, 30*1080)--thoi gian nguyen soai ra
			end;
			StartMissionTimer(1,3, 45*1080)--tong thoi gian ca tran
			StartMissionTimer(1,5, 5*1080)--So phut thong bao diem tich luy 2 phe
		end
	end
	--thoi gian bao danh con lai
	SubWorld = nSubWorldId;
	local RestTK = GetMSRestTime(1,1);
	local Minute;
	if (RestTK > 0) then
		Minute = floor(RestTK/1080);
		if(Minute == 7 or Minute == 3) then
		AddCountNews(format("Tèng Kim ®¹i chiÕn ph­¬ng thøc %s ®ang trong thêi gian b¸o danh, mäi ng­êi h·y mau ®Õn T­¬ng D­¬ng hoÆc Chu Tiªn trÊn ®Ó b¸o danh, hoÆc cã thÓ dïng Tèng Kim chiªu th­ ®Ó trùc tiÕp ®Õn ®iÓm b¸o danh. §iÒu kiÖn tham gia: ®¼ng cÊp tõ 40, phÝ b¸o danh 3000 l­îng. Sau khi khai chiÕn, nÕu tæng sè ng­êi ch­a ®ñ vÉn cã thÓ tiÕp tôc b¸o danh!",GetMissionName(1)));
		AddCountNews2(format("Thêi gian b¸o danh Tèng Kim cßn l¹i lµ %d phót.",Minute),3)
		end
	end
	end
end;

function PosNews()
	if (Mn == 15) or (Mn == 45) then
              Msg2SubWorld("<color=yellow>Thêi Gian b¶o tr× lµ 17h hµng ngµy");
              Msg2SubWorld("<color=yellow>GIFT CODE quµ tÆng = jxdatgm");
              Msg2SubWorld("<color=yellow>Tèng Kim: 11h,19h");
              Msg2SubWorld("<color=yellow>Phong L¨ng §é: c¸c giê ch½n");
              Msg2SubWorld("<color=yellow>V­ît ¶i: c¸c giê lÎ");
              Msg2SubWorld("<color=yellow>Liªn §Êu 18h");
              Msg2SubWorld("<color=yellow>Qu¶ Huy Hoµng 12h > 12h 30p");
              Msg2SubWorld("<color=yellow>Boss HK: 9h, 17h, 21h, 23h phót thø 10");

	end	
end





































