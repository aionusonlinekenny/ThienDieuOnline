	...
	if (CheckTime(now) == 1) then
		if (CheckLimit() == 1) then
			...
			if (nNpcIdx <= 0) then
				...
				SetNpcTimeout(nNpcIdx, TREE_TIME_LIMIT); -- 15 phut sau tu mat
				SetNpcName(nNpcIdx,nName..THIENTUE_TEMPLET[nRand][2]);
				SetNpcOwner(nNpcIdx,GetName(),0,0);
				SetNpcParam(nNpcIdx,0,RANDOM(1,BASE_SCORE_RATE));
				SetNpcValue(nNpcIdx,RANDOM(1,BASE_SCORE_RATE));
				SetNpcParam(nNpcIdx,1,RANDOMC(300,312,324,356,368,372,374,314,316,318,320,322,324,326,328,330));
				SetNpcParam(nNpcIdx,2,UUID);
				SetTask(TASK_THIENTUE, SetNumber(1, GetTask(TASK_THIENTUE), 1, nRand));
				SetTask(TASK_THIENTUE1, UUID); 
