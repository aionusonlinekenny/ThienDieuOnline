...
----------------For blue boss-----------------------------------------------------------
	...
	for i=1,dropnum do
		...
		local nVang = random(1,2)
		AddExtPoint(nVang)
		Msg2Player("<pic=125><color=white>ßπi hi÷p <color=pink>"..GetName().." <color>may mæn nhÀn Æ≠Óc <color=yellow>"..nVang.." Vµng ")
	return end
------------------------------------EVENT------------------------------------------------
-- dropevent(NpcIndex);
-----------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------
	...
	elseif (nSel == 4) then
		...
		if (nRanItem  > 8) then
		nNumLine  = random(1,6); -- so dong cua trang bi xanh
		CalCulator_Drop(NpcIndex,nGenre,nDetail,nParti,nLevel,nSeries,nLuck,nNumLine);
		end
	end
end
