...
function main(nNpcIdx)
	...
	if(hoadang_open() == 1 and (GetWorldKind(w) == 1 or GetWorldKind(w) == 2)) then
		Say("LINK:<npc>",0,STATION_ARRAY[curstationid][3],10,
		"Nh÷ng n¬i ®· ®i qua/noidiqua",
		"Nh÷ng thµnh thÞ ®· ®i qua/chondiadiem",
		"Trë l¹i ®Þa ®iÓm cò /diadiemcu",
		"Khu vùc luyÖn c«ng [c¬ b¶n]/luyencong1",
		"Khu vùc luyÖn c«ng/luyencong2",
		"§i Hoa S¬n c¶nh kü tr­êng(MiÔn phÝ)/hoason",
		"§i TÈy Tñy ®¶o/taytuydao",
		"§i ®Õn n¬i lµm nhiÖm vô d· tÈu/datau",
		"§i b¶n ®å Hoa ®¨ng/hoason",
		"Ta kh«ng muèn ®i/out")
	else
		Say("LINK:<npc>",0,STATION_ARRAY[curstationid][3],10,
		"Nh÷ng n¬i ®· ®i qua/noidiqua",
		...
		"Trë l¹i ®Þa ®iÓm cò /diadiemcu",
		"Khu vùc luyÖn c«ng [c¬ b¶n]/luyencong1",
		"Khu vùc luyÖn c«ng [n©ng cao]/luyencong2(1)",
		"§i Hoa S¬n c¶nh kü tr­êng(MiÔn phÝ)/hoason",
		"§i TÈy Tñy ®¶o/taytuydao",
		"§i ®Õn n¬i lµm nhiÖm vô d· tÈu/datau",
		"Ta kh«ng muèn ®i/out")
...
function thanhthidiqua()
	...
	-- Maps ba lang huyen phai load rieng;
	local iBLH = getn(STATION_ARRAY);
	SetTaskTemp(SEL_MOVEBEGIN+count,iBLH+1);
	StationTab[count+1] = "Ba L¨ng HuyÖn [300 l­îng]/MoveMapBLH";
	count = count + 1;
...
function taykynang()
	...
	local nTotal = nTT1 
	if(nTT1 < 1 and nTT2 < 1 and nTT3 < 1) then
		Talk(1,"","Ng­¬i kh«ng mang theo ®ñ<color=yellow> 3 <color>viªn <color=yellow>Thuû Tinh<color>!")
	return end
	--nTotal = 3;
	local nCityIndex = -1;
	...
	elseif (nCurSW == 176) then
		...
		Msg2Player("Xa phu th«n kh«ng thÓ ®­a c¸c h¹ ®Õn §¶o, <color=yellow> Xa Phu thµnh thÞ míi cã quyÒn h¹n nµy<color>");
		return
	end	
...
function taytiemnang()
	if(GetItemCount(0,4,353) < 6) then
		Talk(1,"","Ng­¬i kh«ng mang theo ®ñ<color=yellow> 6 <color>viªn <color=yellow>Tinh Hång B¶o Th¹ch<color> !")
	return end
	local nCityIndex = -1;
	local nCurSW = GetWorldPos();
	if (nCurSW == 1) then
...
function tayall()
	...
	local nTT2 = GetItemCount(0,4,239)
	local nTT3 = GetItemCount(0,4,240)
              local nTT4 = GetItemCount(0,4,353) 
	if(nTT1 < 1 and nTT2 < 1 and nTT3 < 1) then
		Talk(1,"","Ng­¬i kh«ng mang theo ®ñ<color=yellow> 3 <color>viªn <color=yellow>Thuû Tinh<color>!")
	return end
	if(nTT4 < 6) then
