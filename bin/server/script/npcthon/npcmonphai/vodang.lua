...
local player_Faction = GetFaction();
	...
	elseif (player_Faction == "wudang") then
	     if (GetNumber(1,GetTask(TASK_DUNGCHUNG2),9) == 0) then
			    local szchecklv = GetLevel()
				if (szchecklv >= 60 and szchecklv<80 )then
				...
				elseif szchecklv >= 80 then
				  Say("<npc>: §· l©u kh«ng gÆp! Mäi ng­êi ®Òu rÊt mong nhí! §¹i hiÖp d¹o nµy cã kháe kh«ng?",6,"XuÊt s­ xuèng nói/xuatsu","T×m hiÓu khu vùc luyÖn c«ng/map_help","T×m hiÓu vâ nghÖ bæn m«n/skill_help","Xin chuyÓn dïm lêi hái th¨m s­ phô /no")
				else
