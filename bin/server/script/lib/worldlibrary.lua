--- SERVER_MODE=0 ----- OPEN TEST---------------
--- SERVER_MODE=1 ------ DUA TOP ---------------
----SERVER_MODE=2 ------ Open chinh thuc--------

SERVER_NAME	= "Vâ L©m CTC "
SERVER_MODE = 2   ----chØnh sever teset hoÆc open
SERVER_OPEN = 2104011900 -- yy/mm/dd - hh//mm [21-nam] [03-thang] [14-ngay] [19gio] [00phut]
SERVER_EVENTID = 0
...
function AddQuestKey(nID)
	AddItemID(AddItem(0,4,nID,0,0,5,0,0));
end;

function AddMagicScript(nID)
	AddItemID(AddItem(0,6,nID,0,0,5,0,0));
end;

function DropQuestKey(NpcIndex,nID)
	local nIndex;
	nIndex = AddItem(0,4,nID,0,0,5,0,0);
	DropItem(NpcIndex,nIndex);
end;

function DropMagicScript(NpcIndex,nID)
	local nIndex;
	nIndex = AddItem(0,6,nID,0,0,5,0,0);
	DropItem(NpcIndex,nIndex);
end;
...
function thu123()
local Thu = GetTempInt("DateGame","Thu")
local Ngay = GetTempInt("DateGame","Ngay")
local Thang = GetTempInt("DateGame","Thang")
local Nam = GetTempInt("DateGame","Nam")
if (Thu == 8) then
SetTempInt("DateGame","Thu",2)
	else
SetTempInt("DateGame","Thu",Thu+1)
	end
      end
...
local Thang = GetTempInt("DateGame","Thang")
local Nam = GetTempInt("DateGame","Nam")
if ((Ngay == 31) and (Thang == 4 or 6 or 9 or 11)) then
SetTempInt("DateGame","Ngay",1)
elseif ((Ngay == 32) and (Thang == 1 or 3 or 5 or 7 or 8 or 10 or 12)) then 
SetTempInt("DateGame","Ngay",1)
elseif ((Ngay == 29) and (Thang == 2)) then 
SetTempInt("DateGame","Ngay",1)
        else
SetTempInt("DateGame","Ngay",Ngay+1)
	end 
      end
...
local Nam = GetTempInt("DateGame","Nam")
if (Thang == 13) then
SetTempInt("DateGame","Thang",1)
elseif ((Ngay == 1) and (Thang == 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9 or 10 or 11 or 12)) then
SetTempInt("DateGame","Thang",Thang+1)
       else
       end
     end
...
local Thang = GetTempInt("DateGame","Thang")
local Nam = GetTempInt("DateGame","Nam")	
	if (GetTempInt("DateGame","Thu") == 2) then
	SetTempInt("DateGame","MapCongThanh",1)
	elseif (GetTempInt("DateGame","Thu") == 4) then
	SetTempInt("DateGame","MapCongThanh",78)
	elseif (GetTempInt("DateGame","Thu") == 6) then
	SetTempInt("DateGame","MapCongThanh",37)
	end 
