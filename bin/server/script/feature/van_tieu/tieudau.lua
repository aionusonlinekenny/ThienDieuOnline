...
function batdau()
	...
	return end
	local nTask = GetTask(TASK_RESET6)
	...
	if(GetCash() < 15*1E4) then
		Talk(1,"","Ng­¬i cÇn nép 15 v¹n l­îng phÝ hao tæn tiªu xa!")
	return end
	if(nLan > 2) then
		DelItem(1,0,6,4772)
	else
	SetTask(TASK_RESET6, SetNumber(1,nTask,6,nLan+1))
	end
	local w,x,y = GetWorldPos()
	--local nCamp = GetCurCamp();
	local nName = GetName()
	local n=RANDOMC(0,1,2)
	local nRand = n+1
	local nId = AddNpcNew(2084+n,1,21,2623*32,4503*32,"\\script\\feature\\van_tieu\\tieuxa.lua",0,nil,nil,5,0,500000)
	SetNpcOwner(nId,GetName(),1)
	SetNpcBoss(nId,0);
	SetNpcName(nId,nName..TIEUXA_TEMPLET[nRand][2]);
	SetNpcTimeout(nId,30*1080) ---- NPC mat sau 30p
	SetNpcValue(nId,GetUUID());
	SetTask(TASK_NPCVANTIEU, GetNpcID(nId))
	Msg2Player("log Debug NPC value :"..GetNpcValue(nId).."")
	Msg2Player("log Debug TASK_NPCVANTIEU:"..GetTask(TASK_NPCVANTIEU).."")
...
function hoanthanh()
	local nTaskValue = GetTask(TASK_VANTIEU)
	local nFinish = GetNumber(1,nTaskValue,1)
...
function bicuop()
	...
	if (SubWorld < 0) then
	return end
	Msg2Player("log Debug NPC value :"..GetTask(TASK_NPCVANTIEU).."")
	local nNpcIdx = FindNearNpc(GetTask(TASK_NPCVANTIEU))
	Msg2Player("log Debug FindNearNpc nNpcIdx(bicuop func) :"..nNpcIdx.."")
	if(nNpcIdx > 0) then
		DelNpc(nNpcIdx)
	end
...
function huybo()
	...
	Msg2Player("log Debug NPC value :"..GetTask(TASK_NPCVANTIEU).."")
	local nNpcIdx = FindNearNpc(GetTask(TASK_NPCVANTIEU))
	Msg2Player("log Debug FindNearNpc nNpcIdx(huybo func) :"..nNpcIdx.."")
	if(nNpcIdx > 0) then
		DelNpc(nNpcIdx)
	end
	SetTask(TASK_VANTIEU, 0)
...
function timxe()
	...
	Msg2Player("log Debug NPC value :"..nTask.."")
	local nNpcIdx = FindAroundNpc(nTask)
	Msg2Player("log Debug FindAroundNpc nNpcIdx(timxe func) :"..nNpcIdx.."")
	if (nNpcIdx == 1) then
		Talk(1,"","Kh«ng ph¶i tiªu xa cña ng­¬i ®ang ë ngay ®©y sao!")
	return end
	nNpcIdx = FindNearNpc(nTask)
	Msg2Player("log Debug FindNearNpc nNpcIdx (timxe func) :"..nNpcIdx.."")
	if(nNpcIdx > 0) then
...
function phanthuong(nFinish)
	...
	if(nFinish == 4) then	--hoan thanh dong tieu xa
		...
		AddOwnExp(5*KINH_NGHIEM)
		AddRespect(5)
		AddItemIDStack(AddItem(0,6,4838,0,0,5,0,0),5); --- Ruong van tieu
		Msg2SubWorld("§¹i HiÖp <color=yellow>"..tbName.."<color> giao thµnh c«ng §ong Tiªu Xa nhËn ®­îc<color=yellow> 5 triÖu <color> kinh nghiÖm vµ <color=yellow> 5 <color>®iÓm Uy Danh !")
	...
	elseif(nFinish == 5) then	--hoan thanh bac tieu xa
		AddItemID(AddItem(0,6,4774,0,0,5,0,0),4)
                              AddItemID(AddItem(0,2,34,0,0,5,0,0));    ---- r­¬ng ho¹t ®éng
		AddOwnExp(10*KINH_NGHIEM)
		AddRespect(10)
		AddItemIDStack(AddItem(0,6,4838,0,0,5,0,0),10); --- Ruong van tieu
		Msg2SubWorld("§¹i HiÖp<color=yellow> "..tbName.." <color>giao thµnh c«ng §ong Tiªu Xa nhËn ®­îc<color=yellow> 10 triÖu <color> kinh nghiÖm vµ <color=yellow> 10 <color>®iÓm Uy Danh !")
	...
	elseif(nFinish == 6) then	--hoan thanh vang tieu xa
		AddItemID(AddItem(0,6,4774,0,0,5,0,0),6)
                          AddItemID(AddItem(0,2,34,0,0,5,0,0));    ---- r­¬ng ho¹t ®éng
		AddOwnExp(15*KINH_NGHIEM)
		AddRespect(15)
		AddItemIDStack(AddItem(0,6,4838,0,0,5,0,0),15); --- Ruong van tieu
		Msg2SubWorld("§¹i HiÖp<color=yellow> "..tbName.." <color>giao thµnh c«ng §ong Tiªu Xa nhËn ®­îc<color=yellow> 15 triÖu <color> kinh nghiÖm vµ <color=yellow> 15 <color>®iÓm Uy Danh !")
		...
		AddItemID(AddItem(0,6,4774,0,0,5,0,0),1)
                            AddItemID(AddItem(0,2,34,0,0,5,0,0));    ---- r­¬ng ho¹t ®éng
		AddOwnExp(2*KINH_NGHIEM)
		Msg2SubWorld("§¹i HiÖp<color=yellow> "..tbName.." <color> bÞ c­¬p tiªu kh«ng hoµn thµnh nhiÖm vô !",tbName)
		AddNote(5,2,"Kh«ng hoµn thµnh nhiÖm vô")
	end
end
