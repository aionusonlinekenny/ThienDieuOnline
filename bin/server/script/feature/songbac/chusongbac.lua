Include("\\script\\feature\\songbac\\datasongbac.lua")
Include("\\script\\feature\\songbac\\datanganluong.lua")
Include("\\script\\feature\\songbac\\datatienvang.lua")
...
function nhantien1()
Say("<color=green>Chñ Sßng B¹c<color>: Ng­¬i muèn rót g× ?",3,
"Ta muèn rót Ng©n L­îng th¾ng c­îc /nhantien",
"Ta muèn rót Xu th¾ng cuéc /nhantien",
"Tho¸t./no")
end

function RunTimer12345()
if GetTask(TASK_BAUCUATOM) >= 1 then
RunTimer123456()
SetTask(TASK_BAUCUATOM,0)
else
Msg2Player("B¹n ch­a ®Æt c­îc: <color=green>[BÇu Cua] <color>")
end
end

function RunTimer123456()
thang = tonumber(date("%m"))
gio = tonumber(date("%H"))
phut = tonumber(date("%M"))
ngay = tonumber(date("%d"))

if GetGlbMissionV(11) > 100 then
return end
...
if getn(DataSongBac) == 0 then
            Msg2SubWorld("<color=green>[BÇu Cua]<color> l­ît thø  <color=green>"..GetGlbMissionV(11).." <color>")
SetGlbMission(11, GetGlbMissionV(11) + 1)
return end 


Msg2SubWorld("Danh S¸ch Nh÷ng Ng­êi Tróng Th­ëng, l­ît thø: <color=green>"..GetGlbMissionV(11).." <color>")
...
XacDinh_Vang = 1000000
	...
	for i=1,getn(DataSongBac) do
		...
		if num > 0 then
			...
			if DataSongBac[i][3] == 1 then
				Msg2SubWorld("<color=yellow>["..DataSongBac[i][1].."] <color=red>cã "..num.." con <color=green>"..CheckTen(DataSongBac[i][2]).."<color> nhËn ®­îc: <color=green>"..thutien.." v¹n l­îng")
				...
				Msg2SubWorld("<color=yellow>["..DataSongBac[i][1].."] <color=red> cã "..num.." con <color=green>"..CheckTen(DataSongBac[i][2]).."<color> nhËn ®­îc: <color=yellow>"..thutien.." vµng")
...
function xem()
Msg2Player("L­ît ®Æt c­îc thø: <color=green>"..GetGlbMissionV(11).."<color>")
van_tom = 0
van_cua = 0
van_bau = 0
van_ca = 0
...
if GetGlbMissionV(11) > 100 then
Talk(1,"","§· qu¸ 100 l­ît ch¬i bÇu cua ngµy h«m nay");
return end
...
if GetTaskTemp(1) == 1 then
  Input ("datcuoctienvan",99999999)
else
  Input ("datcuocvang",99999999)
end
end
...
if loai == 1 then
	...
	if vitri == 0 then
		...
		Talk(1,"KickOutSelf","Ng­¬i ®· ®Æt c­îc "..CheckTen(GetTaskTemp(2))..", kh«ng thÓ ®Æt thªm !")
		return
	end
...
function luubang()
	...
	LuuBang("script/feature/songbac/datasongbac.lua",BANG2)
	BANG2 = TaoBang(DataNganLuong,"DataNganLuong")
	LuuBang("script/feature/songbac/datanganluong.lua",BANG2)
	BANG2 = TaoBang(DataTienVang,"DataTienVang")
	LuuBang("script/feature/songbac/datatienvang.lua",BANG2)
end
