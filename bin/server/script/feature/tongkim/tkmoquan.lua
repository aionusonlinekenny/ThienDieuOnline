...
local num = split(GetIP(),":")[1]
            for i=1,getn(CHECK_TK) do
                if (CHECK_TK[i][2] == num) then
                       Talk(1,"","IP b¹n:<color=yellow> "..num.." <color>®· tham gia chiÕn tr­êng [ cÇn gi¶i kÑt ac ®· tham gia ]");
                        Msg2Player("<color=green>mçi IP chØ tham gia 1 ac")
                                                 return end
                                              end
----------------------------------------------------------------------------------------------------------------------------------------------------
...
function gianhaptong()
	...
	if( nPlayerDataIdx > 0) then
		...
		tongcommon(nPlayerDataIdx) --xu ly chi tiet
		AddMissionNote("D¹ng thøc: "..GetMissionName(1)..""," Th«ng C¸o : N¨m 1160 C«ng nguyªn, Kim chñ Hoµn Nhan L­îng tËp hîp qu©n lùc, quyÕt ®Þnh Nam h¹, tiªu diÖt Tèng triÒu. T­¬ng D­¬ng lµ trë ng¹i ®Çu tiªn cña qu©n Kim, T­¬ng D­¬ng chuÈn bÞ ngËp ch×m trong khãi löa binh ®ao. <enter><enter><color=yellow> Nghiªm CÊm : Mäi hµnh vi bug ®iÓm trong Tèng Kim sÏ bÞ xö lý nghiªm kh¾c ! Mong c¸c §¹i HiÖp gióp BQT t¹o nªn mét s©n ch¬i lµnh m¹nh cho mäi ng­êi !")
	end
end;
...
------------------------------------------------------------------------------------------------------------
                      local name = GetAccount()
                      local num = split(GetIP(),":")[1]
                      CHECK_TK[getn(CHECK_TK)+1] = {name,num}
...
-------------------------------------------------------------------------------------------------------------
	...
	Msg2MSAll(1,"<color=yellow> "..GetName().." gia nhËp phe Tèng !"); --thong bao gia nhap
...
-------------------------------------------------------------------------------------------------------------
	...
	Msg2MSAll(1,"<color=yellow> "..GetName().." gia nhËp phe Kim !"); --thong bao gia nhap
	PutMessage(format("B¹n gia nhËp tÇn sè %s[%d]",GetMissionName(1),nGroup));
