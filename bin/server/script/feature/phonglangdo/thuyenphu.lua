...
function lenthuyen()
	...
	if(GetItemCount(0,4,489) < 1) then
		Talk(1,"",12811)
	return end
	local nTaskValue = GetNumber(1,GetTask(TASK_RESET),10) - GetNumber(1,GetTask(TASK_RESET),11);
	if nTaskValue > 3 then
		Talk(1,"","H«m nay ng­¬i ®· ®i thuyÒn 3 lÇn råi, nÕu muèn lªn thuyÒn h·y mang cho ta 1 Kú Hµ Hoµn");
	return end
	...
	SetFightState(0);
	SetPKMode(1,1);--ep kieu luyen cong
	SubWorld = SubWorldID2Idx(337);
	AddMSPlayer(1, 0);
	Msg2MSAll(1,"<color=yellow> "..GetName().." <color>®· lªn thuyÒn !"); --thong bao gia nhap
end;
