...
BOOL KSwordOnLineSever::Init()
{
	...
	*/
    ...
    if (strstr(ModuleName, "gs1.exe")) {
		printf("Started GS1\n");
        iniFile.Load("ServerCfg1.ini");
    } else if (strstr(ModuleName, "gs2.exe")) {
		printf("Started GS2\n");
        iniFile.Load("ServerCfg2.ini");
    } else if (strstr(ModuleName, "gs3.exe")) {
		printf("Started GS3\n");
        iniFile.Load("ServerCfg3.ini");
	} else if (strstr(ModuleName, "gs4.exe")) {
		printf("Started GS4\n");
        iniFile.Load("ServerCfg4.ini");
	} else if (strstr(ModuleName, "gs5.exe")) {
		printf("Started GS5\n");
        iniFile.Load("ServerCfg5.ini");
    } else {
		printf("Started GameServer\n");
        iniFile.Load("ServerCfg.ini");
    }
...
int KSwordOnLineSever::ProcessLoginProtocol(const unsigned long lnID, const char* pData, size_t dataLength)
{
	...
	{
		m_pServer->ShutdownClient(m_pGameStatus[lnID].nPlayerIndex);
		return 0;
	}
	else
	{
