...
#define		defTONG_INIT_MEMBER_SIZE		100		// ≥…‘±ƒ⁄¥Ê≥ı ºªØ ±µƒ¥Û–°
#define		defTONG_MEMBER_SIZE_ADD			100		// ≥…‘±ƒ⁄¥Ê√ø¥Œ‘ˆº”µƒ¥Û–°
...
BOOL	CTongControl::Instate(STONG_INSTATE_COMMAND *pInstate, STONG_INSTATE_SYNC *pSync)
{
	...
	if (channid != -1)
	{
		...
		{
		case enumTONG_FIGURE_DIRECTOR:
			sprintf(szMsg, "%s Æ≠Óc bÊ nhi÷m lµm %s ", szName, defTONG_DIRECTOR_AGNAME);
			break;
		case enumTONG_FIGURE_MANAGER:
			if (pSync->m_btNewFigure > pSync->m_btOldFigure)
				sprintf(szMsg, "%s Æ≠Óc bÊ nhi÷m lµm %s ", szName, defTONG_MANAGER_AGNAME);
			else
				sprintf(szMsg, "%s bﬁ b∑i nhi÷m trÎ thµnh %s ", szName, defTONG_MANAGER_AGNAME);
			break;
		case enumTONG_FIGURE_MEMBER:
			sprintf(szMsg, "%s bﬁ b∑i nhi÷m trÎ thµnh %s ", szName, defTONG_MEMBER_AGNAME);
			break;
		}
...
BOOL	CTongControl::Kick(STONG_KICK_COMMAND *pKick, STONG_KICK_SYNC *pSync)
{
	...
	if (channid != -1)
	{
		sprintf(szMsg, "%s Æ∑ bﬁ trÙc xu t kh·i bÊn bang", szName);
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
	}
...
BOOL	CTongControl::Leave(STONG_LEAVE_COMMAND *pLeave, STONG_LEAVE_SYNC *pSync)
{
	...
	if (channid != -1)
	{
		sprintf(szMsg, "%s chÒ ÆÈng rÍi kh·i bÊn bang", szName);
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
	}
...
BOOL	CTongControl::AcceptMaster(STONG_ACCEPT_MASTER_COMMAND *pAccept)
{
	...
	if (channid != -1)
	{
		sprintf(szMsg, "Bang chÒ %s Æ∑ nh≠Íng ch¯c vﬁ bang chÒ lπi cho %s ", szOldMaster, szName);
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
	}
...
BOOL CTongControl::ChangeMoney(STONG_MONEY_COMMAND* pMoney, STONG_MONEY_SYNC *Sync)
{
	...
	case enumC2S_TONG_MONEY_SAVE:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "%s Æ∑ Æ„ng g„p %d ng©n l≠Óng vµo vµo tr≠¨ng mÙc ng©n khË bÊn bang", pMoney->m_szName,pMoney->m_dwMoney);	// gui tien vao bang
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		}
		break;
	case enumC2S_TONG_MONEY_GET:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "%s Æ∑ rÛt kh·i tr≠¨ng mÙc ng©n khË %d ng©n l≠Óng", pMoney->m_szName,pMoney->m_dwMoney);	// lay tien khoi bang
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		}
		break;
	}
...
BOOL CTongControl::DBChangeAgname(STONG_ACCEPT_AGNAME_COMMAND *pAccept)
{
	...
	if (pAccept->m_btFigure == enumTONG_FIGURE_DIRECTOR)
	{
		...
		sprintf(szMsg, "%s %s Æ≠Óc thay ÆÊi danh hi÷u thµnh: %s ", defTONG_DIRECTOR_AGNAME, m_szDirectorName[i], szAgname);
	}
	else if (pAccept->m_btFigure == enumTONG_FIGURE_MANAGER)
	{
		for (i = 0; i < defTONG_MAX_MANAGER; i++)
		...
		sprintf(szMsg, "%s %s Æ≠Óc thay ÆÊi danh hi÷u thµnh: %s ", defTONG_MANAGER_AGNAME, m_szManagerName[i], szAgname);
	}
	else
	{
		return FALSE;
...
BOOL CTongControl::DBChangeSexAgname(STONG_ACCEPT_SEX_AGNAME_COMMAND *pAccept)
{
	...
	if (pAccept->m_btSex)
	{
		...
		sprintf(szMsg, "Thay ÆÊi danh hi÷u n˜ Æ÷ tˆ trong bang thµnh: %s", szAgname);
	}
	else
	{
		...
		sprintf(szMsg, "Thay ÆÊi danh hi÷u nam Æ÷ tˆ trong bang thµnh: %s", szAgname);
	}
...
BOOL CTongControl::DBChangeCamp(STONG_CHANGE_CAMP_COMMAND *pInfo, STONG_BE_CHANGED_CAMP_SYNC *sInfo)
{
	...
	case camp_justice:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "Phe ph∏i hi÷n tπi cÒa bÊn bang lµ: Ch›nh ph∏i");
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		}
		break;
	case camp_evil:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "Phe ph∏i hi÷n tπi cÒa bÊn bang lµ: Tµ ph∏i");
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		}
		break;
	case camp_balance:
		...
		if (channid != -1)
		{
			sprintf(szMsg, "Phe ph∏i hi÷n tπi cÒa bÊn bang lµ: Trung lÀp");
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		}
		break;
	}
...
BOOL CTongControl::DBChangeTongRecruit(STONG_CHANGE_TONG_INFO_COMMAND *pInfo, STONG_CHANGE_TONG_INFO_SYNC *sInfo)
{
	...
	if (m_bRecruit)
		sprintf(szMsg,"Bang chÒ mÎ ch¯c n®ng chi™u mÈ Æ÷ tˆ ");
	else
		sprintf(szMsg,"Bang chÒ Æ„ng ch¯c n®ng chi™u mÈ Æ÷ tˆ ");
	if (channid != -1)
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
	return TRUE;
}
...
BOOL CTongControl::DBChangeTongLevel(STONG_CHANGE_TONG_INFO_COMMAND *pInfo, STONG_CHANGE_TONG_INFO_SYNC *sInfo)
{
	...
	if (pInfo->m_nValue > 0)
	{
		...
		sprintf(szMsg, "\\O%u", m_dwNameID);
		DWORD channid = g_ChannelMgr.GetChannelID(szMsg, 0);		
		sprintf(szMsg,"ßºng c p bang hÈi hi÷n tπi lµ: %d", m_btLevel);
		if (channid != -1)
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
		return TRUE;
	}
...
BOOL CTongControl::DBChangeTongMoney(STONG_CHANGE_TONG_INFO_COMMAND *pInfo, STONG_CHANGE_TONG_INFO_SYNC *sInfo)
{
	...
	if (pInfo->m_nValue > 0)
	{
		...
		{
			sprintf(szMsg, "<color=255,107,107>Ti“n thu thu’ bang hÈi chi’m l‹nh hi÷n tπi Æ≠Óc %d l≠Óng<color>", pInfo->m_AddMoneyThue); //TamLTM thong bao tien thu thue bang hoi chiem linh
		}
		else
		{
			sprintf(szMsg, "Ng©n qu¸ bang hÈi hi÷n tπi lµ %d l≠Óng", m_dwMoney); // Set ngan quy bang hoi hien tai
		}
...
BOOL CTongControl::DBChangeTongEff(STONG_CHANGE_TONG_INFO_COMMAND *pInfo, STONG_CHANGE_TONG_INFO_SYNC *sInfo)
{
	...
	if (pInfo->m_nValue > 0)
	{
		...
		sprintf(szMsg, "\\O%u", m_dwNameID);
		DWORD channid = g_ChannelMgr.GetChannelID(szMsg, 0);
		sprintf(szMsg, "CËng hi’n tÂn bang hÈi hi÷n tπi lµ %d Æi”m", m_dwTotalEff);
		if (channid != -1)
			g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));	
		return TRUE;
	}
...
BOOL CTongControl::DBChangeJiyu(STONG_CHANGE_TONG_INFO_COMMAND *pInfo, STONG_CHANGE_TONG_INFO_SYNC *sInfo)
{
	...
	if (channid != -1)
	{
		sprintf(szMsg,"Bang chÒ Æ∑ ti’n hµnh B∂o l≠u tin t¯c bang hÈi");
		g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
	}
	return TRUE;
}
		...
		case enumC2S_CHANGE_TONG_MEMBEREFF:
		{
			...
			switch(pInfo->m_btFigure)
			{
				...
				if (channid != -1)
				{
					if(pInfo->m_nValue2)
					{
					sprintf(szMsg, "%s Æ∑ Æ„ng g„p %d Æi”m cËng hi’n vµo tr≠¨ng mÙc cËng hi’n bÊn bang", pInfo->m_szName, pInfo->m_nValue2);
					g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string(MESSAGE_TONG_ANNOUCE_HEAD), std::string(szMsg));
					}
					TMemberStruct	sMember;
