        ...
        case WND_N_RIGHT_CLICK_ITEM:
			{
				...
				else
				{
					...
					{
						MoveItemToRepository(pItem);
						return TRUE;
					}
					OnClickItem(pItem, true);
				}
				break;
			}
...
void KUiItem::MoveItemToRepository(KUiDraggedObject* pItem)
{
    ...
    if (pStoreBox)
    {
        ...
        if (curPage >= numOpened)
        {
            ...
            Msg.eType = SMT_NORMAL;
            Msg.uReservedForUi = 0;
            strcpy(Msg.szMessage, "Trang r­¬ng chøa nµy ch­a ®­îc më !");
            KUiSysMsgCentre::AMessageArrival(&Msg, NULL);
            return;
        }

        if (containerID < UOC_STORE_BOX || containerID > UOC_STORE_BOX10)
        {
            KSystemMessage Msg;
            Msg.byConfirmType = SMCT_NONE;
            Msg.byParamSize = 0;
    ...
    g_DebugLog("[CLIENT] MoveItemToRepository: uId=%d, Pos=(%d,%d), Size=(%d x %d), SrcCont=%d, DestCont=%d",
        pItem->uId, Pick.Region.h, Pick.Region.v, Pick.Region.Width, Pick.Region.Height,
        Pick.eContainer, Drop.eContainer);
