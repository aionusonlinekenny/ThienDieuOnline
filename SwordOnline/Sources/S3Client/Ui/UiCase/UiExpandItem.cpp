    ...
    switch (uMsg) {
        ...
        case WND_N_RIGHT_CLICK_ITEM:
            ...
            else
            {
                ...
                if (!pItem || pItem->uGenre != CGOG_ITEM)
                    break;
                ...
                {
                    int containerID = KUiStoreBox::GetIfVisible()->GetCurrentContainerID();
                    Drop.eContainer = (UIOBJECT_CONTAINER)containerID;
                }
                else
                {
                ...
                g_DebugLog("[CLIENT] RightClick ExpandEquip: Move uId=%d from UOC_ITEM_TAKE_WITH_EX ? %d", pItem->uId, Drop.eContainer);
