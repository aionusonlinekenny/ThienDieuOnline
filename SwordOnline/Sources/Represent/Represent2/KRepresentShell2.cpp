...
//¸ù¾ÝSPRÍ·Ö¸Õë£¬»ñÈ¡µ÷É«°æ»º³åÇøÖ¸Õë
...
//##ÊÍ·Å¶Ô(Í¨¹ýGetBitmapDataBufferµ÷ÓÃ»ñÈ¡µÃµÄ)Í¼ÐÎÏñµãÊý¾Ý»º³åÇøµÄ¿ØÖÆ
...
//## ·µ»ØÖ¸¶¨×ø±êÔÚ×Ö·û´®ÖÐ×î½üµÄ×Ö·ûÆ«ÒÆ
...
/*
void KRepresentShell2::DrawSpriteScreenBlendSSE2(
    BYTE byInputAlpha,
    DWORD dwMask32,
    void* pBuffer,
    INT width,
...
)
{
    ...
    // Tính toán vùng dích
    sDestRect.left = max(rc1.left, rc2.left);
    sDestRect.top = max(rc1.top, rc2.top);
    sDestRect.right = min(rc1.right, rc2.right);
    sDestRect.bottom = min(rc1.bottom, rc2.bottom);

    // Tính các thông s? clip
    INT clipX = sDestRect.left - nX + sSrcRect.left;
    INT clipY = sDestRect.top - nY + sSrcRect.top;
    INT clipWidth = sDestRect.right - sDestRect.left;
    INT clipHeight = sDestRect.bottom - sDestRect.top;

    nX = sDestRect.left;
    nY = sDestRect.top;

    INT sprSkip = nWidth * clipY + clipX;
    INT sprSkipPerLine = nWidth - clipWidth;
    INT nClipRight = nWidth - clipX - clipWidth;
    INT bufSkip = nPitch * nY + nX * 2;
    INT nBufSkipPerLine = nPitch - clipWidth * 2;
    ...
    // Nh?y d?n di?m b?t d?u
    _Jmp(sprSkip);
    pDst = (WORD*)((BYTE*)pDst + bufSkip);

    // Render t?ng dòng
    for (INT nCurrentY = 0; nCurrentY < clipHeight; nCurrentY++)
    {
        INT x = clipWidth;
        WORD* pDstLine = pDst + (nCurrentY * (nPitch / 2) + sDestRect.left - nX + 13);
        ...
        while (x - 4 >= 0)
        {
            ...
            // X? lý t?ng pixel gi? nguyên logic g?c
            if (_Alpha())
            {
                src_color[0] = _Color();
            ...
            // X? lý 4 hu?ng
            if (nCurrentY < clipHeight / 4) {
                // Hu?ng 12 gi?
                ScreenBlend4Pixel(pDstLine, src_color, src_alpha, byInputAlpha);
            }
            else if (nCurrentY < clipHeight / 2) {
                // Hu?ng 4 gi?
                ScreenBlend4Pixel(pDstLine, src_color, src_alpha, byInputAlpha);
            }
            else if (nCurrentY < clipHeight * 3 / 4) {
                // Hu?ng 6 gi?
                ScreenBlend4Pixel(pDstLine, src_color, src_alpha, byInputAlpha);
            }
            else {
        ...
        if (x - 2 >= 0)
        {
            unsigned short src_color[2] = { 0 };
            unsigned short src_alpha[2] = { 0 };
            ...
            // X? lý 4 hu?ng cho 2 pixel
            if (nCurrentY < clipHeight / 4) {
                // Hu?ng 12 gi?
                ScreenBlend2Pixel(pDstLine, src_color, src_alpha, byInputAlpha);
            }
            else if (nCurrentY < clipHeight / 2) {
                // Hu?ng 4 gi?
                ScreenBlend2Pixel(pDstLine, src_color, src_alpha, byInputAlpha);
        ...
        // X? lý pixel cu?i cùng
        if (x - 1 >= 0)
        {
            unsigned short src_color[1] = { 0 };
            unsigned short src_alpha[1] = { 0 };
            ...
            // X? lý 4 hu?ng cho pixel cu?i
            if (nCurrentY < clipHeight / 4) {
                // Hu?ng 12 gi?
                if (src_color[0] != 0)
                    ScreenBlend1Pixel(pDstLine, src_color[0], src_alpha[0], byInputAlpha);
            }
            else if (nCurrentY < clipHeight / 2) {
                // Hu?ng 4 gi?
                if (src_color[0] != 0)
        ...
        // Di chuy?n d?n dòng ti?p theo
        if (nCurrentY < clipHeight - 1)
        {
            _Jmp(sprSkipPerLine + 1);
        }
    }
}*/