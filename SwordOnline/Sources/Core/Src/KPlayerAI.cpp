...
BOOL KPlayerAI::MoveMps() // Cai nay move theo toa do
{
	...
	{
		m_nRadiusAuto = 500; // radius gi quen roi
		if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_FightMode == enumFightActive)
		{
...
BOOL KPlayerAI::ReturnFromPortal()
{
    ...
    case Step_Training:
        m_bSellItem = TRUE;
		strcpy(sMsg.szMessage, "->Thao t¸c khi trong thµnh vui lßng ®îi");
    ...
    case Step_SellTrash:
    if (m_bSellTrash && m_bSellItem && !m_bStopAutoBanRac)
    {
		sprintf(sMsg.szMessage, "->.Thùc hiÖn b¸n vËt phÈm.");
		Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].SetChatInfo("Auto", sMsg.szMessage, strlen(sMsg.szMessage));
        m_DelayTime++;
        m_nSellTrashTimeout++; 
		
        if (!m_bSellingTrashNow)
        {
            SendClientAutoSellItemCmd(12);
            m_bSellingTrashNow = TRUE;
            m_DelayTime = 0;
        }

        if (m_bSellingTrashNow && m_DelayTime >= 20)
        {
			sprintf(sMsg.szMessage, "->.B¸n vËt phÈm kh«ng cã trong läc thuéc tÝnh vui lßng ®îi.");
			Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].SetChatInfo("Auto", sMsg.szMessage, strlen(sMsg.szMessage));
            BanRacKhongLoi();

            if (m_bStopAutoBanRac)
            {
                m_bSellItem = FALSE;
                m_nSellTrashTimeout = 0; 
                m_nReturnPortalStep = Step_Repair;
				sprintf(sMsg.szMessage, "->.§· b¸n xong r¸c chuyÓn sang s÷a ch÷a ®å h­.");
				Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].SetChatInfo("Auto", sMsg.szMessage, strlen(sMsg.szMessage));
                g_DebugLog(">>> [ReturnFromPortal] Da ban xong rac, chuyen buoc Step_Repair\n");
            }
        }

        
        if (m_nSellTrashTimeout > 200) 
        {
			sprintf(sMsg.szMessage, "->.Qua thêi gian cho phÐp b¸n r¸c tù trë l¹i n¬i luyÖn c«ng.");
			Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].SetChatInfo("Auto", sMsg.szMessage, strlen(sMsg.szMessage));
            g_DebugLog(">>> [ReturnFromPortal] Qua thoi gian ban rac, chuyen sang Step_Repair\n");
            m_bSellItem = FALSE;
            m_bStopAutoBanRac = TRUE;
            m_bSellingTrashNow = FALSE;
            m_nSellTrashTimeout = 0;
            m_nReturnPortalStep = Step_Repair;
        }
    }
    ...
    case Step_InventoryMoney:
        ...
        {
            int amount = Player[CLIENT_PLAYER_INDEX].m_ItemList.GetEquipmentMoney();
            Player[CLIENT_PLAYER_INDEX].m_ItemList.ExchangeMoney(room_equipment, room_repository, amount);
        }
        m_nReturnPortalStep = Step_WithDrawaMoney;
        break;

    case Step_WithDrawaMoney:
        if (m_bWithDrawa)
        {
    ...
    case Step_Return:
        if (m_bReturnPortal && KSG_GetCurSec() >= m_nCurReturnPortalSec)
        {
            m_DelayTime = 0;
            m_bSellTrash = TRUE;
            m_bSellingTrashNow = FALSE;
            m_bStopAutoBanRac = FALSE;
            nPlaceRoom = 0;
			sprintf(sMsg.szMessage, "->Quay l¹i n¬i luyÖn c«ng, tiÕp tôc train.");
			Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].SetChatInfo("Auto", sMsg.szMessage, strlen(sMsg.szMessage));
            PLAYER_COMMAND sRP;
            ...
            g_DebugLog(">>> [ReturnFromPortal] Hoan tat cac buoc, bat dau tro lai luyen cong\n");
...
/*void KPlayerAI::BanRacKhongLoi()
{
    ...
    while (pItem)
    {
        ...
        {
            KLockItem* pLock = Item[nIdx].GetLock();
            if (pLock && pLock->nState != LOCK_STATE_NORMAL)
            {
                pItem = Player[CLIENT_PLAYER_INDEX].m_ItemList.GetNextItem();
                continue;
            }
            ...
            if (
                ...
                (m_bSaveJewelry && CheckEquip(Item[nIdx].GetDetailType())) ||
                ...
                Item[nIdx].GetDetailType() == equip_horse ||
                Item[nIdx].GetKind() == gold_item ||
                Item[nIdx].GetKind() == purple_item ||
    ...
    if (nTargetIdx > 0)
    {
        int soLuong = Item[nTargetIdx].GetStackNum();
        if (soLuong > 0)
        {
            SendClientCmdSell(Item[nTargetIdx].GetID(), soLuong);
            return;
        }
    }
    m_bStopAutoBanRac = TRUE;
...
void KPlayerAI::BanRacKhongLoi()
{
    if (m_bStopAutoBanRac)
    {
        g_DebugLog(">>> [BanRac] Da dung auto ban rac, khong thuc hien nua\n");
        return;
    }

    if (Player[CLIENT_PLAYER_INDEX].m_ItemList.IsLockOperation())
    {
        g_DebugLog(">>> [BanRac] Dang xu ly ban truoc do, bo qua\n");
        return;
    }
    ...
    while (pItem)
    {
        ...
        if (pItem->nPlace == pos_equiproom && nIdx > 0 && nIdx < MAX_ITEM)
        {
            ...
            if (pLock && pLock->nState != LOCK_STATE_NORMAL)
            {
                g_DebugLog(">>> [BanRac] Item ID = %d bi khoa, bo qua\n", Item[nIdx].GetID());
                pItem = Player[CLIENT_PLAYER_INDEX].m_ItemList.GetNextItem();
                continue;
            }

            // B? qua các lo?i không nên bán
            if (
                CheckEquipMagic(nIdx) ||
                (m_bSaveJewelry && CheckEquip(Item[nIdx].GetDetailType())) ||
                Item[nIdx].GetGenre() == item_task ||
    ...
    if (nTargetIdx > 0)
    {
        ...
        {
            SendClientCmdSell(Item[nTargetIdx].GetID(), soLuong);
            g_DebugLog(">>> Ban rac: ID = %d, SL = %d\n", Item[nTargetIdx].GetID(), soLuong);
        }
        return;
    }
    m_bStopAutoBanRac = TRUE;
    g_DebugLog(">>> [BanRac] Khong con item nao de ban, dung AutoBanRac\n");


    m_nReturnPortalStep = Step_Repair;
    g_DebugLog(">>> [BanRac] Chuyen sang buoc tiep theo: Step_Repair\n");
}
