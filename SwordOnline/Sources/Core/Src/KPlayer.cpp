...
void	KPlayer::SendSelfTeamInfo()
{
	if (m_cTeam.m_nFlag && m_cTeam.m_nID >= 0)	// ·¢ËÍ¶ÓÎéÐÅÏ¢
	{
		...
		strcpy(sSelfInfo.m_szNpcName[0], Npc[nNpcIndex].Name);		// ¶ÓÔ±
		for (int i = 0; i < MAX_TEAM_MEMBER; i++)
		{
			if (g_Team[m_cTeam.m_nID].m_nMember[i] >= 0)
			{
				...
				sSelfInfo.m_SM[i + 1] = Player[g_Team[m_cTeam.m_nID].m_nMember[i]].m_cFaction.m_nCurFaction;  // Lay HP thành viên
    ...
    if (nRet == 0)
    {
        ...
        switch(nRet)
        {
            case equip_horse:
                DesPos.nPlace = 2;
                DesPos.nX = 10;
                strcpy(sMsg.szMessage, "Mang <color=yellow>Ngùa<color> ...");
                break;
            case equip_meleeweapon:
                DesPos.nPlace = 2;
                DesPos.nX = 3;
				strcpy(sMsg.szMessage, "Mang <color=yellow>Vò khÝ cËn chiÕn<color> ...");
                break;
            case equip_rangeweapon:
                DesPos.nPlace = 2;
                DesPos.nX = 3;
				strcpy(sMsg.szMessage, "Mang <color=yellow>Vò khÝ tÇm xa<color> ...");
                break;
            case equip_armor:
                DesPos.nPlace = 2;
                DesPos.nX = 1;
                strcpy(sMsg.szMessage, "Mang <color=yellow>Y phôc<color> ...");
                break;
            case equip_ring:
                DesPos.nPlace = 2;
                DesPos.nX = 7;
                strcpy(sMsg.szMessage, "Mang <color=yellow>NhÉn 1<color> ...");
				if(m_ItemList.m_EquipItem[7] > 0 && m_ItemList.m_EquipItem[8] == 0) {
					DesPos.nX = 8;
					strcpy(sMsg.szMessage, "Mang <color=yellow>NhÉn 2<color> ...");
				}
            ...
            case equip_amulet:
                DesPos.nPlace = 2;
                DesPos.nX = 6;
                strcpy(sMsg.szMessage, "Mang <color=yellow>D©y chuyÒn<color> ...");
                break;
            case equip_boots:
                DesPos.nPlace = 2;
                DesPos.nX = 4;
                strcpy(sMsg.szMessage, "Mang <color=yellow>Giµy<color> ...");
                break;
            case equip_belt:
                DesPos.nPlace = 2;
                DesPos.nX = 2;
                strcpy(sMsg.szMessage, "Mang <color=yellow>§ai l­ng<color> ...");
                break;
            case equip_helm:
                DesPos.nPlace = 2;
                DesPos.nX = 0;
				strcpy(sMsg.szMessage, "Mang <color=yellow>Mò<color> ...");
                break;
            case equip_cuff:
                DesPos.nPlace = 2;
                DesPos.nX = 5;
                strcpy(sMsg.szMessage, "Mang <color=yellow>Bao tay<color> ...");
                break;
            case equip_pendant:
                DesPos.nPlace = 2;
                DesPos.nX = 9;
                strcpy(sMsg.szMessage, "Mang <color=yellow>Ngäc béi<color> ...");
                break;
        }
...
BOOL KPlayer::ServerPickUpItem(BYTE* pProtocol)
{
	...
	int nObjIndex = ObjSet.FindID(pPickUp->m_nObjID);
	if (nObjIndex == 0)
		return FALSE;

	// --- Các ki?m tra quy?n s? h?u (?ã gi? nguyên nh? c?) ---
	if (Object[nObjIndex].m_nBelong != -1)
	{
		...
		else
		{
			if (!g_Team[m_cTeam.m_nID].CheckIn(m_nPlayerIndex, Object[nObjIndex].m_nBelong))
			{
				SHOW_MSG_SYNC sMsg;
				sMsg.ProtocolType = s2c_msgshow;
				sMsg.m_wMsgID = (Object[nObjIndex].m_nKind == Obj_Kind_Money)
	...
	{
		DWORD dwScriptId = Object[nObjIndex].m_dwScriptID;
		...
		if (pScript)
		{
			try
			{
				...
				pScript->CallFunction(NORMAL_FUNCTION_NAME, 0, "d", nObjIndex);
	...
	// --- X? lý nh?t ti?n ---
	else if (Object[nObjIndex].m_nKind == Obj_Kind_Money)
	{
		int nMoneyMapT = 0;
		int mMapAddMoney = 11; // fix c?ng: thành ?ô

		if (mMapAddMoney != 0 && SubWorld[mMapAddMoney].m_bCheckTong)
		{
			int nTongT = 5;
		...
		int nEarnedMoney = Object[nObjIndex].m_nMoneyNum - nMoneyMapT;
		if (nEarnedMoney < 0) nEarnedMoney = 0;

		if (!Earn(nEarnedMoney))
			return FALSE;

		// N?u c?n: g?i lua chia ti?n cho bang
		if (mMapAddMoney != 0 && SubWorld[mMapAddMoney].m_bCheckTong)
		{
			try
			{
				...
				if (pScript)
				{
					int nTopIndex = 0;
					pScript->SafeCallBegin(&nTopIndex);
					pScript->CallFunction("AddMoneyMain", 0, "dsd",
				...
				printf("Exception when executing banghoi.lua!\n");
			}
		}
...
#ifndef _SERVER
void FormatNumberWithDot(int number, char* buffer, size_t size)
{
    if (!buffer || size < 2)
    {
        if (buffer && size > 0)
    ...
    int finalLen = lstrlen(result);
    if ((int)size <= finalLen)
    {
        buffer[0] = '\0';
        return;
    }
...
void	KPlayer::s2cLevelUp(BYTE* pMsg)
{
    ...
    {/*
        ...
        CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
		*/
		int nLevelGain = PlayerSet.m_cLevelAdd.GetLevelExp(
		Npc[m_nIndex].m_Level,
		Npc[m_nIndex].m_byTranslife
		) - m_nExp;

		char szFormattedExp[32];
		FormatNumberWithDot(nLevelGain, szFormattedExp, sizeof(szFormattedExp));

		KSystemMessage sMsg;
		sprintf(sMsg.szMessage, MSG_GET_EXP, szFormattedExp);  // MSG_GET_EXP ph?i dùng %s
		sMsg.eType = SMT_NORMAL;
		sMsg.byConfirmType = SMCT_NONE;
		sMsg.byPriority = 0;
		sMsg.byParamSize = 0;
...
void	KPlayer::s2cSetExp(int nExp)
{
    if (nExp > m_nExp)
    {
        ...
        CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
		*/
		int nExpGain = nExp - m_nExp;

		char szFormattedExp[32];
		FormatNumberWithDot(nExpGain, szFormattedExp, sizeof(szFormattedExp));

		KSystemMessage sMsg;
		sprintf(sMsg.szMessage, MSG_GET_EXP, szFormattedExp);  // ?? ??m b?o MSG_GET_EXP dùng %s
    ...
    else if (nExp < m_nExp)
    {
        ...
        CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
		*/
		int nExpLost = m_nExp - nExp;

		char szFormattedExp[32];
		FormatNumberWithDot(nExpLost, szFormattedExp, sizeof(szFormattedExp));

		KSystemMessage sMsg;
		sprintf(sMsg.szMessage, MSG_DEC_EXP, szFormattedExp);  // ?? MSG_DEC_EXP ph?i dùng %s
...
{
    // m_ItemList.RemoveAll(); // tmphucgl: todo
