...
KProtocolProcess::KProtocolProcess()
{
	...
	ProcessFunc[s2c_RightItemAutoMove] = s2cRightItemAutoMove;
	...
	//end code
	ProcessFunc[s2c_extpointsync] = s2cExtPointValueSync; //TamLTM fix xu;
	...
	ProcessFunc[s2c_finishquest] = OpenDTBox;//TamLTM da tau vng
	ProcessFunc[s2c_otherbox] = s2cOpenOtherBox; //TamLTM kham
	ProcessFunc[s2c_openprogressbar] = OpenProgressBar; //TamLTM open progress bar
	ProcessFunc[s2c_syncposmin] = SysNcPosMin; //TamLTM fix lag pos
...
void KProtocolProcess::SyncCurNormalData(BYTE* pMsg)
{
	...
	{
	#ifndef _SERVER
    KNpc& player = Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex];
    int newLife = pSync->m_shLife;
	if (player.m_LastLife == -1)
    {
        player.m_LastLife = newLife;
    }
    else if (player.m_LastLife > newLife)
    {
        int damage = player.m_LastLife - newLife;
        player.AddBloodPlayerDame(damage, false);
        player.m_LastLife = newLife;
    }
    else if (player.m_LastLife < newLife)
    {
        int heal = newLife - player.m_LastLife;
        player.AddBloodPlayerDame(heal, true);
        player.m_LastLife = newLife;
    }
    else
    {
...
void KProtocolProcess::SyncNpcMin(BYTE* pMsg)
{
	...
	//	g_DebugLog("Npc Doing lag pos 2");
		...
		if (nIdx != Player[CLIENT_PLAYER_INDEX].m_nIndex)
		{
			...
			Npc[nIdx].m_CurrentLifeMax	= NpcSync->CurrentLifeMax;
			Npc[nIdx].m_CurrentLife		= NpcSync->CurrentLife;
			Npc[nIdx].m_bPendingCritical = NpcSync->m_bNpcGotCritDamage;
...
void KProtocolProcess::s2cRightItemAutoMove(BYTE* pMsg)
{
	...
	ItemPos src, dest;
	src.nPlace = p->SrcPlace;
	src.nX = p->SrcX;
	src.nY = p->SrcY;
	dest.nPlace = p->DestPlace;
	dest.nX = p->DestX;
	dest.nY = p->DestY;

	Player[CLIENT_PLAYER_INDEX].m_ItemList.HandleAutoMoveFromServer(src, dest);
	

    int nRoom = Player[CLIENT_PLAYER_INDEX].m_ItemList.PositionToRoom(dest.nPlace);
    if (nRoom >= 0)
    {
        int nItemID = Player[CLIENT_PLAYER_INDEX].m_ItemList.m_Room[nRoom].GetIndex(dest.nX, dest.nY);
        if (nItemID > 0 && nItemID < MAX_ITEM && Item[nItemID].GetID() > 0)
        {
            const char* szName = Item[nItemID].GetName();
            if (szName && szName[0])
            {
                KSystemMessage msg;
                ...
                msg.byPriority = 0;
                msg.byParamSize = 0;
                sprintf(msg.szMessage, MSG_MOVE_ITEM_HANH_TRANG, szName);
...
{
	ITEM_RIGHT_AUTO_MOVE_REQ* p = (ITEM_RIGHT_AUTO_MOVE_REQ*)pProtocol;
	if (!p) return;

	KPlayer& player = Player[nIndex];
	KItemList& itemList = player.m_ItemList;

	int nRoom = itemList.PositionToRoom(p->SrcPlace);
	if (nRoom < 0 || nRoom >= room_num)
	return;

	int nItemID = itemList.m_Room[nRoom].GetIndex(p->SrcX, p->SrcY);
	if (nItemID <= 0)
	return;
	itemList.AutoMoveToRepository(nItemID, p->SrcX, p->SrcY, p->SrcPlace, p->DestPlace);
}
