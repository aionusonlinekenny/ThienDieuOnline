...
case GDI_ITEM_IN_STORE_BOX6:
		...
		if (uParam)
		{
			...
			pInfo++;
			PlayerItem* pItem = Player[CLIENT_PLAYER_INDEX].m_ItemList.GetFirstItem();
			if (pItem && pItem->nPlace == pos_repositoryroom6)
			{
				pInfo->Obj.uGenre = CGOG_ITEM;
				pInfo->Obj.uId = pItem->nIdx;
				pInfo->Region.h = pItem->nX;
	...
	case GDI_ITEM_IN_STORE_BOX7:
		...
		if (uParam)
		{
			...
			pInfo++;
			PlayerItem* pItem = Player[CLIENT_PLAYER_INDEX].m_ItemList.GetFirstItem();
			if (pItem && pItem->nPlace == pos_repositoryroom7)
			{
				pInfo->Obj.uGenre = CGOG_ITEM;
				pInfo->Obj.uId = pItem->nIdx;
				pInfo->Region.h = pItem->nX;
	...
	case GDI_ITEM_IN_STORE_BOX8:
		...
		if (uParam)
		{
			...
			pInfo++;
			PlayerItem* pItem = Player[CLIENT_PLAYER_INDEX].m_ItemList.GetFirstItem();
			if (pItem && pItem->nPlace == pos_repositoryroom8)
			{
				pInfo->Obj.uGenre = CGOG_ITEM;
				pInfo->Obj.uId = pItem->nIdx;
				pInfo->Region.h = pItem->nX;
	...
	case GDI_ITEM_IN_STORE_BOX9:
		...
		if (uParam)
		{
			...
			pInfo++;
			PlayerItem* pItem = Player[CLIENT_PLAYER_INDEX].m_ItemList.GetFirstItem();
			if (pItem && pItem->nPlace == pos_repositoryroom9)
			{
				pInfo->Obj.uGenre = CGOG_ITEM;
				pInfo->Obj.uId = pItem->nIdx;
				pInfo->Region.h = pItem->nX;
	...
	case GDI_ITEM_IN_STORE_BOX10:
		...
		if (uParam)
		{
			...
			pInfo++;
			PlayerItem* pItem = Player[CLIENT_PLAYER_INDEX].m_ItemList.GetFirstItem();
			if (pItem && pItem->nPlace == pos_repositoryroom10)
			{
				pInfo->Obj.uGenre = CGOG_ITEM;
				pInfo->Obj.uId = pItem->nIdx;
				pInfo->Region.h = pItem->nX;
...
//--------------------------------------------------------------------------
int PosFromContainer(int eContainer)
{
    switch (eContainer)
    {
    case UOC_ITEM_TAKE_WITH:    return pos_equiproom;
    case UOC_ITEM_TAKE_WITH_EX: return pos_equiproomex;
    case UOC_STORE_BOX:         return pos_repositoryroom;
    case UOC_STORE_BOX1:        return pos_repositoryroom1;
    case UOC_STORE_BOX2:        return pos_repositoryroom2;
    case UOC_STORE_BOX3:        return pos_repositoryroom3;
    case UOC_STORE_BOX4:        return pos_repositoryroom4;
    case UOC_STORE_BOX5:        return pos_repositoryroom5;
    case UOC_STORE_BOX6:        return pos_repositoryroom6;
    case UOC_STORE_BOX7:        return pos_repositoryroom7;
    case UOC_STORE_BOX8:        return pos_repositoryroom8;
    case UOC_STORE_BOX9:        return pos_repositoryroom9;
    case UOC_STORE_BOX10:       return pos_repositoryroom10;
    default:                    return -1;
    }
}
	...
	case GOI_SWITCH_OBJECT:
		...
		//nParam = (KUiObjAtContRegion*)pObject2 -> ·ÅÏÂµÄÎïÆ·²Ù×÷ºóµÄÐÅÏ¢
		{
			...
			if (pObject2 && pObject2->Region.h == -1 && pObject2->Region.v == -1)
			{
				...
				else if (pObject1->eContainer == UOC_ITEM_TAKE_WITH_EX)
					nPlace = pos_equiproomex;
				else if (pObject1->eContainer >= UOC_STORE_BOX && pObject1->eContainer <= UOC_STORE_BOX10)
				...
				else if (pObject2->eContainer == UOC_ITEM_TAKE_WITH_EX)
					nDestPlace = pos_equiproomex;
				else if (pObject2->eContainer >= UOC_STORE_BOX && pObject2->eContainer <= UOC_STORE_BOX10)
