...
#include "../../Represent/Represent2/KRepresentShell2.h"
#include "ImgRef.h"
#include "Text.h"
#include "KOption.h"
#endif
...
#define MAX_BLOOD_FLOAT_HEIGHT 			200
//end code

#define		BLOOD_EVENTTIME				100
...
//Son fix out pt
        ...
        if (IsPlayer() && Npc[m_nLastDamageIdx].IsPlayer())
        {
			...
			if (m_nPlayerIdx > 0)
			{
			sprintf(szMessageDeath,"B¹n ®· bÞ <color=yellow>%s<color> ®¸nh träng th­¬ng!!!",Npc[m_nLastDamageIdx].Name);
			...
			{
				sprintf(szMessageDeath,"B¹n ®· ®¸nh träng th­¬ng <color=yellow>%s<color>!!!",Name);
...
void KNpc::NormalSync()
{
	...
	NpcSync.m_bNpcGotCritDamage = m_bNpcGotCritDamage; // Goi flag chi mang ve client
	m_bNpcGotCritDamage = FALSE; // Reset lai sau khi goi
...
//	RUIconImageR.oPosition.nX = nMpsX - 50;
	...
	if (len != 0)
	{
		sprintf(Processing, "%s ®ang cõu s¸t b¹n", m_nNamePK);
		int	 nFontSize = 12;
...
//--------------------------------------------------------------------------
//	Â¹Â¦Ã„ÃœÂ£ÂºÃ”Ã¶Â¼Ã“ÂµÂ±Ã‡Â°Ã—Ã®Â´Ã³Ã‰ÃºÃƒÃ¼ÂµÃ£
//--------------------------------------------------------------------------
...
//--------------------------------------------------------------------------
//	Â¹Â¦Ã„ÃœÂ£ÂºÃ”Ã¶Â¼Ã“Â»Ã¹Â±Â¾Ã—Ã®Â´Ã³ÃŒÃ¥ÃÂ¦ÂµÃ£
//--------------------------------------------------------------------------
...
//--------------------------------------------------------------------------
//	Â¹Â¦Ã„ÃœÂ£ÂºÃ”Ã¶Â¼Ã“ÂµÂ±Ã‡Â°Ã—Ã®Â´Ã³ÃŒÃ¥ÃÂ¦ÂµÃ£
//--------------------------------------------------------------------------
...
//--------------------------------------------------------------------------
//	Â¹Â¦Ã„ÃœÂ£ÂºÃ”Ã¶Â¼Ã“Â»Ã¹Â±Â¾Ã—Ã®Â´Ã³Ã„ÃšÃÂ¦ÂµÃ£
//--------------------------------------------------------------------------
...
void KNpc::GetNpcCopyFromTemplate(int nNpcTemplateId)
{
	...
	if (g_pNpcTemplate[nNpcTemplateId]) //ÃŠÃ½Â¾ÃÃ“ÃÃÂ§Ã”Ã²Â¿Â½Â±Â´Â£Â¬Â·Ã±Ã”Ã²Ã–Ã˜ÃÃ‚Ã‰ÃºÂ³Ã‰
...
int KNpc::ChangeWorld(DWORD dwSubWorldID, int nX, int nY)
{
	...
	if (-1 == nTargetSubWorld && IsPlayer())
	{
		if (m_SubWorldIndex >= 0 && m_RegionIndex >= 0) {
			SubWorld[m_SubWorldIndex].m_MissionArray.RemovePlayer(m_nPlayerIdx, SubWorld[m_SubWorldIndex].m_SubWorldID);
...
void KNpc::AddBlood(int nNo)
{
    if (nNo > 0)
    {
        if (m_bPendingCritical)
        {
            sprintf(m_szBlood[m_btCurBlood], "!CRIT%d", nNo);
            m_bPendingCritical = FALSE;
        }
        else
        {
...
void KNpc::ClearBlood(int iSlot)
{
    if (iSlot < 0 || iSlot >= BLOOD_COUNT)
        return;

    m_szBlood[iSlot][0] = '\0';
    m_nBlood[iSlot] = 0;
    m_nBloodAlpha[iSlot] = 0;
}
void KNpc::AddBloodPlayerDame(int nValue, bool bHeal)
{
    if (nValue <= 0 || nValue >= 100000)
        return;
    char prefix = bHeal ? '+' : '-';
    char suffix = bHeal ? 'h' : 'd';
    char szText[16] = { 0 };
    sprintf(szText, "%c%d", prefix, nValue);  // VD: +123 ho?c -456
    m_nBlood[m_btCurBlood] = BLOOD_EVENTTIME;
    strncpy(m_szBlood[m_btCurBlood], szText, sizeof(m_szBlood[m_btCurBlood]) - 1);
    m_btCurBlood++;
    if (m_btCurBlood >= BLOOD_COUNT)
        m_btCurBlood = 0;
}

int KNpc::PaintPlayerDameBlood(int nHeightOffset)
{
    for (int i = 0; i < BLOOD_COUNT; ++i)
    {
        if (!m_szBlood[i][0] || m_nBlood[i] <= 0)
            continue;

        const char* bloodStr = m_szBlood[i];
        int len = strlen(bloodStr);
        if (len <= 1)
            continue;

        BOOL isHeal = (bloodStr[0] == '+');
        BOOL isDamage = (bloodStr[0] == '-');
        char suffix = isHeal ? 'h' : 'd';
        float lifeRatio = (float)m_nBlood[i] / BLOOD_EVENTTIME;
        float speedFactor = 0.2f + i * 0.2f;
        float progress = powf(1.0f - lifeRatio, speedFactor);
        progress = min(max(progress, 0.0f), 1.0f);

        int nHeightOff = (int)(progress * MAX_BLOOD_FLOAT_HEIGHT);
        int nMpsX, nMpsY;
        GetMpsPos(&nMpsX, &nMpsY);

        KImageParam ParamArray[16];
        char sprFiles[16][64];
        int totalWidth = 0;

        // Load sprite prefix
        KImageParam ParamSymbol;
        memset(&ParamSymbol, 0, sizeof(ParamSymbol));
        if (isHeal)
            g_pRepresent->GetImageParam("\\Spr\\Ui3\\bloodtext\\plush.spr", &ParamSymbol, ISI_T_SPR);
        else if (isDamage)
            g_pRepresent->GetImageParam("\\Spr\\Ui3\\bloodtext\\minusd.spr", &ParamSymbol, ISI_T_SPR);

        if (ParamSymbol.nNumFrames)
            totalWidth += ParamSymbol.nWidth;

        // Load digits
        for (int j = 1; j < len; ++j)
        {
            sprintf(sprFiles[j], "\\Spr\\Ui3\\bloodtext\\%c%c.spr", bloodStr[j], suffix);
            g_pRepresent->GetImageParam(sprFiles[j], &ParamArray[j], ISI_T_SPR);
            totalWidth += ParamArray[j].nWidth;
        }
        int startX = nMpsX - totalWidth / 2;
        int baseY = nMpsY - 25;
        int offsetY = 4;
        ...
        {
            KRUImage symbolImg;
			memset(&symbolImg, 0, sizeof(symbolImg));
            strncpy(symbolImg.szImage, isHeal ? "\\Spr\\Ui3\\bloodtext\\plush.spr" : "\\Spr\\Ui3\\bloodtext\\minusd.spr", sizeof(symbolImg.szImage) - 1);
            symbolImg.nType = ISI_T_SPR;
            symbolImg.Color.Color_b.a = alpha;
            symbolImg.bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
            symbolImg.bRenderFlag = RUIMAGE_RENDER_FLAG_REF_SPOT;
            symbolImg.oPosition.nX = startX;
            symbolImg.oPosition.nY = baseY - ParamSymbol.nHeight / 2 - offsetY - nHeightOff;
            g_pRepresent->DrawPrimitives(1, &symbolImg, RU_T_IMAGE, FALSE);
            startX += ParamSymbol.nWidth;
        }

        // Draw digits
        for (j = 1; j < len; ++j)
        {
            const KImageParam& param = ParamArray[j];
            if (!param.nNumFrames)
                continue;

            KRUImage digitImg;
			memset(&digitImg, 0, sizeof(digitImg));
            strncpy(digitImg.szImage, sprFiles[j], sizeof(digitImg.szImage) - 1);
            digitImg.nType = ISI_T_SPR;
            digitImg.Color.Color_b.a = alpha;
            digitImg.bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
            digitImg.bRenderFlag = RUIMAGE_RENDER_FLAG_REF_SPOT;
            digitImg.oPosition.nX = startX;
            digitImg.oPosition.nY = baseY - param.nHeight / 2 - offsetY - nHeightOff;
            g_pRepresent->DrawPrimitives(1, &digitImg, RU_T_IMAGE, FALSE);
            startX += param.nWidth;
        }
...
int KNpc::PaintBlood(int nHeightOffset)
{
    ...
    if (IsPlayer())
    {
        nHeightOffset += PaintPlayerDameBlood(nHeightOffset);
        return nHeightOffset;
    }
#endif
    ...
    for (int i = 0; i < BLOOD_COUNT; i++)
    {
        ...
        float lifeRatio = (float)m_nBlood[i] / BLOOD_EVENTTIME;
        float speedFactor = 0.2f + i * 0.2f;  // Slot sau trôi ch?m hon
        float progress = powf(1.0f - lifeRatio, speedFactor);
        progress = min(max(progress, 0.0f), 1.0f);
        ...
        // --- RÕ LÚC Ð?U, M? D?N ---
        float fadeStart = 0.3f;  // Gi? alpha cao trong 30% th?i gian d?u
        float fadeProgress = (progress - fadeStart) / (1.0f - fadeStart);
        fadeProgress = min(max(fadeProgress, 0.0f), 1.0f);
        BYTE alpha = (BYTE)(255 * (1.0f - fadeProgress));

        // === DAMAGE CHÍ M?NG ===
        if (strncmp(bloodStr, "!CRIT", 5) == 0)
        {
            const char* pDamage = bloodStr + 5;
            int len = strlen(pDamage);
            if (len <= 0)
                continue;

            int totalWidth = 0;
            KImageParam critParam;
            memset(&critParam, 0, sizeof(KImageParam));
            g_pRepresent->GetImageParam("\\Spr\\Ui3\\bloodtext\\crit.spr", &critParam, ISI_T_SPR);
            if (critParam.nNumFrames > 0)
                totalWidth += critParam.nWidth;

            KImageParam ParamArray[16];
            char sprFiles[16][64];
            for (int j = 0; j < len; j++)
            {
                wsprintf(sprFiles[j], "\\Spr\\Ui3\\bloodtext\\%c.spr", pDamage[j]);
                memset(&ParamArray[j], 0, sizeof(KImageParam));
                g_pRepresent->GetImageParam(sprFiles[j], &ParamArray[j], ISI_T_SPR);
                totalWidth += ParamArray[j].nWidth;
            }
            ...
            // V? crit.spr TRU?C s? damage, cao hon 4px
            if (critParam.nNumFrames > 0)
            {
                KRUImage critImg;
                memset(&critImg, 0, sizeof(KRUImage));
                lstrcpyn(critImg.szImage, "\\Spr\\Ui3\\bloodtext\\crit.spr", sizeof(critImg.szImage) - 1);
                critImg.nType = ISI_T_SPR;
                critImg.Color.Color_b.a = min(255, alpha + 51);  // Tang d? rõ hon 20%
                critImg.bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
                critImg.bRenderFlag = RUIMAGE_RENDER_FLAG_REF_SPOT;
                critImg.oPosition.nX = startX;
                critImg.oPosition.nY = baseY - critParam.nHeight / 2 - offsetY - nHeightOff - 4; // cao hon 4px
                critImg.oPosition.nZ = 0;
                critImg.nFrame = g_SubWorldSet.GetGameTime() % critParam.nNumFrames;
                g_pRepresent->DrawPrimitives(1, &critImg, RU_T_IMAGE, FALSE);

                startX += critParam.nWidth;
            }

            for (j = 0; j < len; j++)
            {
                if (ParamArray[j].nNumFrames <= 0)
                    continue;

                KRUImage digitImg;
                memset(&digitImg, 0, sizeof(KRUImage));
                lstrcpyn(digitImg.szImage, sprFiles[j], sizeof(digitImg.szImage) - 1);
                digitImg.nType = ISI_T_SPR;
                digitImg.Color.Color_b.a = alpha;
                digitImg.bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
                digitImg.bRenderFlag = RUIMAGE_RENDER_FLAG_REF_SPOT;
                digitImg.oPosition.nX = startX;
                digitImg.oPosition.nY = baseY - ParamArray[j].nHeight / 2 - offsetY - nHeightOff;
                digitImg.oPosition.nZ = 0;
                digitImg.nFrame = g_SubWorldSet.GetGameTime() % ParamArray[j].nNumFrames;
                g_pRepresent->DrawPrimitives(1, &digitImg, RU_T_IMAGE, FALSE);
                startX += ParamArray[j].nWidth;
            }
        ...
        for (int j = 0; j < len; j++)
        {
            wsprintf(sprFiles[j], "\\Spr\\Ui3\\bloodtext\\%c.spr", bloodStr[j]);
            memset(&ParamArray[j], 0, sizeof(KImageParam));
            g_pRepresent->GetImageParam(sprFiles[j], &ParamArray[j], ISI_T_SPR);
            totalWidth += ParamArray[j].nWidth;
        }
