...
#define		BLOOD_COUNT					6

#define BLOOD_MOVE_SPEED        1
#define BLOOD_FADE_BASE_ALPHA   255
//	跑动速度与行走速度的倍数
...
public:
	int					nTotalReceiveDamage;
	BOOL 				m_bLastDamageCritical;
	BOOL 				m_bNpcGotCritDamage;						   
...
private:
	...
	int  				m_nCritDamage[BLOOD_COUNT];
	int  				m_nCritTime[BLOOD_COUNT];
	BYTE 				m_btCurCrit;
    //son fix loi ket map
...
//	void				CheckTimerMoveBarrier();
	...
	void				SetSex(int nSex);// 设定此 npc 的五行属性（内容还没完成）
...
//end code
	...
	// 冒血处理
	BOOL 				m_bPendingCritical; 						  
	void 				AddBloodPlayerDame(int nValue, bool bHeal);
	void				AddBlood(int nNo);
	int 				PaintPlayerDameBlood(int nHeightOffset);
