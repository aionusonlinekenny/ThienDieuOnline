...
#endif
#ifndef _SERVER
/*BOOL KItemList::HandleAutoMoveFromServer(ItemPos sSrc, ItemPos sDest)
{
    g_DebugLog("[CLIENT] HandleAutoMoveFromServer Src=(%d,%d,%d) Dest=(%d,%d,%d)",
        sSrc.nPlace, sSrc.nX, sSrc.nY, sDest.nPlace, sDest.nX, sDest.nY);
    ...
    {
        g_DebugLog("[CLIENT] ERROR: Invalid source or destination room");
        return FALSE;
    }

    int nItemID = m_Room[nSrcRoom].GetIndex(sSrc.nX, sSrc.nY);
    if (nItemID <= 0)
    {
        g_DebugLog("[CLIENT] ERROR: No item found at source (%d,%d)", sSrc.nX, sSrc.nY);
        return FALSE;
    }

    int width = Item[nItemID].GetWidth();
    int height = Item[nItemID].GetHeight();

    // Th?c hi?n chuy?n item trong room
    if (!m_Room[nSrcRoom].PickUpItem(nItemID, sSrc.nX, sSrc.nY, width, height))
    {
        g_DebugLog("[CLIENT] ERROR: PickUpItem failed");
        return FALSE;
    }

    if (!m_Room[nDestRoom].PlaceItem(sDest.nX, sDest.nY, nItemID, width, height))
    {
        g_DebugLog("[CLIENT] ERROR: PlaceItem failed, rolling back...");
        m_Room[nSrcRoom].PlaceItem(sSrc.nX, sSrc.nY, nItemID, width, height);
        return FALSE;
    }
    ...
    if (idx <= 0)
    {
        g_DebugLog("[CLIENT] ERROR: FindSame failed for item %d", nItemID);
        return FALSE;
    }
    ...
    srcRegion.Obj.uGenre = CGOG_ITEM;
    srcRegion.Obj.uId = nItemID;
    srcRegion.Region.Width = width;
    srcRegion.Region.Height = height;
    srcRegion.Region.h = sSrc.nX;
    srcRegion.Region.v = sSrc.nY;
    srcRegion.eContainer = (UIOBJECT_CONTAINER)MapPlaceToUIContainer(sSrc.nPlace);

    destRegion = srcRegion;
    destRegion.Region.h = sDest.nX;
    destRegion.Region.v = sDest.nY;
...
BOOL KItemList::AutoMoveToRepository(int nItemID, int nSrcX, int nSrcY, int nSrcPlace, int nDestPlace)
{
    ...
    if (nItemID <= 0 || nItemID >= MAX_ITEM || !Item[nItemID].GetID())
        return FALSE;
    ...
    if (m_Room[nSrcRoom].GetIndex(nSrcX, nSrcY) != nItemID)
        return FALSE;

    POINT pt;
    if (!m_Room[nDestRoom].FindRoom(width, height, &pt))
        return FALSE;

    if (!m_Room[nSrcRoom].PickUpItem(nItemID, nSrcX, nSrcY, width, height))
        return FALSE;

    if (!m_Room[nDestRoom].PlaceItem(pt.x, pt.y, nItemID, width, height))
    {
        m_Room[nSrcRoom].PlaceItem(nSrcX, nSrcY, nItemID, width, height);
        return FALSE;
    }
...
int KItemList::MapPlaceToUIContainer(int nPlace)
{
    ...
    if (nPlace == pos_equiproomex)
        return UOC_ITEM_TAKE_WITH_EX;
    if (nPlace >= pos_repositoryroom && nPlace < pos_repositoryroom + 10)
        return UOC_STORE_BOX + (nPlace - pos_repositoryroom);
...
int KItemList::PositionToRoom(int nPlace)
{
    ...
    else if (nPlace == pos_equiproomex)
