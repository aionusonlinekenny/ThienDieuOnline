...
BOOL KItem::SetAttrib_MA(IN const KItemNormalAttrib* pMA)
{
    ...
    {
        m_aryMagicAttrib[i].nAttribType = pMA[i].nAttribType;
        m_aryMagicAttrib[i].nValue[0] = pMA[i].nValue[0];
        m_aryMagicAttrib[i].nValue[1] = pMA[i].nValue[1];
        m_aryMagicAttrib[i].nValue[2] = pMA[i].nValue[2];
        m_aryMagicAttrib[i].nMin = pMA[i].nMin;  // Gán giá tr? min
        m_aryMagicAttrib[i].nMax = pMA[i].nMax;  // Gán giá tr? max
...
{
    // Kiem tra neu khong co loai thuoc tính thì bo qua
    if (!m_aryMagicAttrib[i].nAttribType)
    {
        // Phan hien the "Chua kham nam" neu loai là NATURE_VIOLET
        if (m_CommonAttrib.nItemNature == NATURE_VIOLET && i < m_GeneratorParam.nLuck)
       {
            if (i == MAX_ITEM_NORMAL_MAGICATTRIB)
                strcat(pszMsg, "\n");
    ...
    // Lay mo ta thuoc tính tu g_MagicDesc
    char* pszTemp2 = (char *)g_MagicDesc.GetDesc(&m_aryMagicAttrib[i]);
    if (!pszTemp2 || !pszTemp2[0])
        continue;
    // Them dong trong neu can cho loai thuoc tính dac biet
    if (i == MAX_ITEM_NORMAL_MAGICATTRIB && m_aryMagicAttrib[i].nAttribType)
        strcat(pszMsg, "\n");

    // Kiem tra va them mau sac cho cac thuoc tinh
    if (i < MAX_ITEM_NORMAL_MAGICATTRIB)
    {
        if ((i & 1) == 0)
        {
    ...
    else 
    {
        ...
        {
            sprintf(pszMsg, "%s <color=0,255,255>[<color=0,255,0>%d<color=0,255,255>-%d]", pszMsg, m_aryMagicAttrib[i].nMin, m_aryMagicAttrib[i].nMax);
        } 
        else 
        {
            sprintf(pszMsg, "%s <color=0,255,255>[%d-%d]", pszMsg, m_aryMagicAttrib[i].nMin, m_aryMagicAttrib[i].nMax);
        }
    }

    // Xuong dong và dat lai mau sac cho cac thuoc tính tiep theo
    strcat(pszMsg, "\n");
    strcat(pszMsg, "<color=255,255,255>");
}
